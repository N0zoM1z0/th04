...
| MAINE.EXE | 72 | 24 / 41 / 7 | 18 | 54 | Five SCORE_TEXT owners accepted; inspect adjacent places dispatcher |
| ZUN.COM | 13 | 13 / 0 / 0 | 0 | 11, plus 2 blocked | `_main` natural CALL shape; ZUNINIT/MEMCHK source authority |

The non-MAIN authored backlog is 146 pending acceptances, 15 provisional
boundaries, and 38 unresolved `target-derived-asm` candidate representations.
...
172-byte name cursor renderer, and 184-byte score-entry row renderer;
...
Five MAINE SCORE functions bring MAINE to 18 / 1,887; the OP stage renderer
...
- v549 accepts MAINE SCORE_TEXT's score-entry row renderer at `1A05:26C1`, payload `0xC711..0xC7C8`, 184 bytes ending `RET 4`. Its two standalone near-call displacement words carry OMF fixups; all other CODE bytes, grouped SCORE_TEXT, and both linked target functions match. Candidate EXE/MAP and 559 ordered relocations remain stable. The decoded wrapper covers 18 MAINE slices; packed-file and whole-TU exactness remain open. Next review `places_put` at `0xC7C9`. See the [SCORE note](reconstruction/op-maine/TH04_MAINE_SCORE_CPP_V479.md#v549-maintained-score-entry-row-renderer).
