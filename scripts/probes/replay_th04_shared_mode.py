#!/usr/bin/env python3
"""Cold-relink OP and MAINE with the maintained shared sound-mode detection TU.
...
SOURCE = ROOT / "src/shared/sound/determine_modes.cpp"
...
ARTIFACTS = {
    "op": (0xDCE4, 0x9C, 804, "v228-op-target-roundtrip/a/restored.bin",
    ...
    "maine": (0xCFAA, 0x9C, 559, "v228-maine-target-roundtrip/a/restored.bin",
...
def contribution(path: Path) -> tuple[int, int, str]:
    pattern = re.compile(
        ...
        r"\bS=SHARED\b.*\bM=th04/snd_mode\.cpp(?:\s|$)", re.I,
    )
    matches = []
    ...
    if len(matches) != 1:
        raise RuntimeError(f"expected one sound-mode MAP contribution: {path}")
    return matches[0]
...
def main() -> int:
    ...
    for name, (start, size, reloc_count, target_rel, target_sha, baseline_sha) in ARTIFACTS.items():
        ...
        if len(targets[name].program_image[start:start + size]) != size:
            raise RuntimeError(f"{name}: truncated target sound-mode function")
    ...
    for label in ("a", "b"):
        ...
        for name, (start, size, reloc_count, _, _, baseline_sha) in ARTIFACTS.items():
            ...
            shutil.copy2(SOURCE, work / "th04/snd_mode.cpp")
            ...
            obj = work / "obj/th04/snd_mode.obj"
            obj.unlink()
            command = ["wine", str(RUNNER), "-e", "-x", "tcc", "-c", "-I.",
                       "-O", "-b-", "-3", "-Z", "-d", "-DGAME=4", "-ml",
                       "-nobj/th04/", "th04/snd_mode.cpp"]
            ...
            if differences or candidate.program_image != baselines[name].program_image:
                raise RuntimeError(f"{label}/{name}: sound-mode function or aggregate program differs")
    ...
    receipt = {
        ...
        "claim_scope": "artifact-local decoded sound-mode function raw replay",
