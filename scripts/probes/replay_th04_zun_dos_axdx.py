#!/usr/bin/env python3
"""Cold-replace ZUN's DOS_AXDX library member after local DOS_FREE support."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local DOS_AXDX changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local DOS_AXDX changed MAP")
...
def main() -> int:
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned DOS_AXDX support replaces external library member after local DOS_FREE support; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "DOS_AXDX is library-origin support; its trailing module NOP is alignment outside the 0x15-byte function owner. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
