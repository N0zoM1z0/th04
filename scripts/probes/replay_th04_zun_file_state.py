#!/usr/bin/env python3
"""Cold-replace ZUN's FIL data/BSS library member after local VERSION/GRP support."""
...
LOCAL_FONTOPEN = ROOT / "src/shared/dos/dos_ropen.asm"
FONTOPEN_TARGET_CODE_SHA256 = "3442d8129f35ad11432cf42da08320cff50c6473a4540457f82a2e43f8a188dd"
FONTOPEN_TARGET_DATA_SHA256 = "96a296d224f285c67bee93c30f8a309157f0daa35dc5b87e410b78630a09cfc7"
FONTOPEN_OMF_CODE_SHA256 = "292d9f139fa2d096f2606edff3d59228025c42a5da34e17db638a512ef318f51"
FONTOPEN_OMF_DATA_SHA256 = "96a296d224f285c67bee93c30f8a309157f0daa35dc5b87e410b78630a09cfc7"
FONTOPEN_LINK_RELEVANT_OMF_SHA256 = "7468e2b53697ba2426ba9c394e79510246da348935d80c95f49f024031900ac9"
FONTOPEN_MEMBER_POSITION = 337
FONTOPEN_TARGET_CODE_OFFSET = 0x136C
FONTOPEN_TARGET_DATA_OFFSET = 0x2212
FONTOPEN_CODE_SIZE = 0x18
FONTOPEN_DATA_SIZE = 0x02
FONTOPEN_CANDIDATE_CODE_OFFSET = 0x7FE
FONTOPEN_CANDIDATE_DATA_OFFSET = 0x16AA
LOCAL_VERSION = ROOT / "src/shared/runtime/master_version.asm"
VERSION_DATA_SHA256 = "4d3224b4a9686c99aa134fdce59b2fc11d6576931ee8351573a97433789cd4ba"
VERSION_LINK_RELEVANT_OMF_SHA256 = "aa110fcf787d004e35e9e16e27ce31954a26d32651467c9d0ce6da50232ff0bc"
VERSION_MEMBER_POSITION = 0
VERSION_TARGET_DATA_OFFSET = 0x219A
VERSION_DATA_SIZE = 0x5D
VERSION_TARGET_PADDING_OFFSET = 0x21F7
VERSION_CANDIDATE_DATA_OFFSET = 0x1632
VERSION_CANDIDATE_PADDING_OFFSET = 0x168F
LOCAL_GRP = ROOT / "src/shared/hardware/graph_state.asm"
GRP_DATA_SHA256 = "76f94dec73b8bddc3f93da96b0cddfe96880b05680b4d8f4899825f2f348e2ab"
GRP_LINK_RELEVANT_OMF_SHA256 = "162a24b820624c37dcf701a91c6baf0ea34be15b82fc4af0714a56bb2b79cfb7"
GRP_MEMBER_POSITION = 91
GRP_TARGET_DATA_OFFSET = 0x21F8
GRP_DATA_SIZE = 0x0B
GRP_TARGET_PADDING_OFFSET = 0x2203
GRP_CANDIDATE_DATA_OFFSET = 0x1690
GRP_CANDIDATE_PADDING_OFFSET = 0x169B
LOCAL_FILE_STATE = ROOT / "src/shared/dos/file_state.asm"
FILE_STATE_DATA_SHA256 = "b7d1b3a1104cc86b1cea310793cf777002db0517281d135a02de079b0ea87c23"
FILE_STATE_LINK_RELEVANT_OMF_SHA256 = "70ec111491abefe0181bec46a21a994ab636dbe87991b5ce85a56363726d311b"
FILE_STATE_MEMBER_POSITION = 163
FILE_STATE_TARGET_DATA_OFFSET = 0x220E
FILE_STATE_DATA_SIZE = 0x04
FILE_STATE_BSS_SIZE = 0x14
FILE_STATE_CANDIDATE_DATA_OFFSET = 0x16A6
...
def fontopen_object_sections(path: Path) -> tuple[bytes, bytes]:
    records = parse_omf(path.read_bytes())
    sections = [record.data[3:] for record in records if record.record_type == 0xA0
                and record.data[1:3] == b"\0\0"]
    if sorted(map(len, sections)) != [FONTOPEN_DATA_SIZE, FONTOPEN_CODE_SIZE]:
        raise RuntimeError("FONTOPEN OMF LEDATA layout changed")
    code = next(data for data in sections if len(data) == FONTOPEN_CODE_SIZE)
    data = next(data for data in sections if len(data) == FONTOPEN_DATA_SIZE)
    return code, data


def extdef_names(path: Path) -> tuple[str, ...]:
    names = []
    for record in parse_omf(path.read_bytes()):
        ...
        while offset < len(data):
            ...
            if offset + length > len(data):
                raise RuntimeError(f"malformed EXTDEF in {path}")
            names.append(data[offset:offset + length].decode("ascii"))
            offset += length
            if offset >= len(data):
                raise RuntimeError(f"missing EXTDEF type index in {path}")
            if data[offset] & 0x80:
                if offset + 1 >= len(data):
                    raise RuntimeError(f"truncated EXTDEF type index in {path}")
                offset += 2
            else:
                offset += 1
    return tuple(names)
...
def pure_data_object(path: Path, size: int, name: str) -> bytes:
    records = parse_omf(path.read_bytes())
    sections = [record.data[3:] for record in records if record.record_type == 0xA0
                and record.data[1:3] == b"\0\0"]
    if len(sections) != 1 or len(sections[0]) != size:
        raise RuntimeError(f"{name} OMF DATA layout changed")
    if sum(record.record_type == 0x98 for record in records) != 2:
        raise RuntimeError(f"{name} must retain empty _TEXT plus _DATA SEGDEF")
    return sections[0]


def file_state_object(path: Path) -> bytes:
    records = parse_omf(path.read_bytes())
    sections = [record.data[3:] for record in records if record.record_type == 0xA0
                and record.data[1:3] == b"\0\0"]
    if len(sections) != 1 or len(sections[0]) != FILE_STATE_DATA_SIZE:
        raise RuntimeError("FIL OMF DATA layout changed")
    seg_sizes = sorted(
        int.from_bytes(record.data[1:3], "little")
        for record in records if record.record_type == 0x98
    )
    if seg_sizes != [0, FILE_STATE_DATA_SIZE, FILE_STATE_BSS_SIZE]:
        raise RuntimeError(f"FIL SEGDEF size topology changed: {seg_sizes!r}")
    return sections[0]


def map_equal_except_bufferpos_alias_order(
    original_map: bytes, local_map: bytes
) -> bool:
    if original_map == local_map:
        return True
    original = original_map.decode("cp437").splitlines()
    local = local_map.decode("cp437").splitlines()
    if len(original) != len(local):
        return False
    differences = [i for i, (a, b) in enumerate(zip(original, local)) if a != b]
    ...
    return (
        {original[i], original[j]} == expected
        and {local[i], local[j]} == expected
        and original[i] == local[j]
        and original[j] == local[i]
    )
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        sources = (("GRPCLEAR", LOCAL_GRAPH_CLEAR), ("RESDATA", LOCAL_RESDATA),
                   ...
                   ("FONTOPEN", LOCAL_FONTOPEN), ("VERSION", LOCAL_VERSION),
                   ("GRP", LOCAL_GRP), ("FIL", LOCAL_FILE_STATE))
        ...
        local_fontopen_code, local_fontopen_data = fontopen_object_sections(saved / "fontopen.obj")
        if sha(local_fontopen_code) != FONTOPEN_OMF_CODE_SHA256:
            raise RuntimeError(f"{label}: local FONTOPEN OMF CODE changed")
        if sha(local_fontopen_data) != FONTOPEN_OMF_DATA_SHA256:
            raise RuntimeError(f"{label}: local FONTOPEN OMF DATA changed")
        if link_relevant_omf_sha(saved / "fontopen.obj") != FONTOPEN_LINK_RELEVANT_OMF_SHA256:
            raise RuntimeError(f"{label}: local FONTOPEN link-relevant OMF changed")
        local_version = pure_data_object(saved / "version.obj", VERSION_DATA_SIZE, "VERSION")
        if sha(local_version) != VERSION_DATA_SHA256:
            raise RuntimeError(f"{label}: local VERSION DATA changed")
        if link_relevant_omf_sha(saved / "version.obj") != VERSION_LINK_RELEVANT_OMF_SHA256:
            raise RuntimeError(f"{label}: local VERSION link-relevant OMF changed")
        local_grp = pure_data_object(saved / "grp.obj", GRP_DATA_SIZE, "GRP")
        if sha(local_grp) != GRP_DATA_SHA256:
            raise RuntimeError(f"{label}: local GRP DATA changed")
        if extdef_names(saved / "grp.obj") != ("_Master_Version",):
            raise RuntimeError(f"{label}: local GRP lost VERSION force-link EXTDEF")
        if link_relevant_omf_sha(saved / "grp.obj") != GRP_LINK_RELEVANT_OMF_SHA256:
            raise RuntimeError(f"{label}: local GRP link-relevant OMF changed")
        local_file_state = file_state_object(saved / "fil.obj")
        if sha(local_file_state) != FILE_STATE_DATA_SHA256:
            raise RuntimeError(f"{label}: local FIL DATA changed")
        if link_relevant_omf_sha(saved / "fil.obj") != FILE_STATE_LINK_RELEVANT_OMF_SHA256:
            raise RuntimeError(f"{label}: local FIL link-relevant OMF changed")
        ...
        if (len(names) != 640 or names.count("filread") != 1
                ...
                or names.count("filclose") != 1 or names.count("fontopen") != 1
                or names.count("version") != 1 or names.count("grp") != 1
                or names.count("fil") != 1):
        ...
        original_fontopen = work / "fontopen.OBJ"
        original_fontopen_code, original_fontopen_data = fontopen_object_sections(original_fontopen)
        if original_fontopen_code != local_fontopen_code or original_fontopen_data != local_fontopen_data:
            raise RuntimeError(f"{label}: local FONTOPEN LEDATA differs from original member")
        (saved / "fontopen-original.obj").write_bytes(original_fontopen.read_bytes())
        original_version = work / "version.OBJ"
        original_version_data = pure_data_object(original_version, VERSION_DATA_SIZE, "VERSION")
        if original_version_data != local_version:
            raise RuntimeError(f"{label}: local VERSION LEDATA differs from original member")
        (saved / "version-original.obj").write_bytes(original_version.read_bytes())
        original_grp = work / "grp.OBJ"
        original_grp_data = pure_data_object(original_grp, GRP_DATA_SIZE, "GRP")
        if original_grp_data != local_grp:
            raise RuntimeError(f"{label}: local GRP LEDATA differs from original member")
        if extdef_names(original_grp) != ("_Master_Version",):
            raise RuntimeError(f"{label}: original GRP VERSION force-link EXTDEF drift")
        (saved / "grp-original.obj").write_bytes(original_grp.read_bytes())
        original_file_state = work / "fil.OBJ"
        original_file_state_data = file_state_object(original_file_state)
        if original_file_state_data != local_file_state:
            raise RuntimeError(f"{label}: local FIL DATA differs from original member")
        (saved / "fil-original.obj").write_bytes(original_file_state.read_bytes())
        ...
        if order.index("fontopen") != FONTOPEN_MEMBER_POSITION:
            raise RuntimeError(f"{label}: FONTOPEN library member moved")
        if order.index("version") != VERSION_MEMBER_POSITION:
            raise RuntimeError(f"{label}: VERSION library member moved")
        if order.index("grp") != GRP_MEMBER_POSITION:
            raise RuntimeError(f"{label}: GRP library member moved")
        if order.index("fil") != FILE_STATE_MEMBER_POSITION:
            raise RuntimeError(f"{label}: FIL library member moved")
        ...
        shutil.copyfile(saved / "fontopen.obj", original_fontopen)
        shutil.copyfile(saved / "version.obj", original_version)
        shutil.copyfile(saved / "grp.obj", original_grp)
        shutil.copyfile(saved / "fil.obj", original_file_state)
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local FIL changed component bytes")
        if not map_equal_except_bufferpos_alias_order(original_map, map_data):
            raise RuntimeError(f"{label}: local FIL changed MAP beyond same-address BufferPos alias order")
        map_alias_order_only = map_data != original_map
        ...
        expected = (
            ...
            "0000:08FE 0018 C=CODE", "M=fontopen", "0000:17AA 0002 C=DATA",
            "0000:08FE       DOS_ROPEN", "0000:08FE idle  FONTFILE_OPEN",
            "0000:17AA idle  file_sharingmode",
            "0000:051A 0000 C=CODE", "M=version", "0000:1732 005D C=DATA",
            "0000:1732 idle  _Master_Version_NEAR", "0000:1746       _Master_Version",
            "0000:174B idle  _Master_Copyright",
            "0000:051A 0000 C=CODE", "M=grp", "0000:1790 000B C=DATA",
            "0000:1790       _graph_VramSeg", "0000:1792       _graph_VramWords",
            "0000:05D8 0000 C=CODE", "M=fil", "0000:17A6 0004 C=DATA",
            "0000:19DA 0014 C=BSS", "0000:17A6       file_BufferSize",
            "0000:17A8       file_Handle", "0000:19DA idle  file_Pointer",
        )
        ...
        return {
            ...
            "map_bufferpos_alias_order_only": map_alias_order_only,
            ...
            "fontopen_member_position": order.index("fontopen"),
            "fontopen_original_object_sha256": sha((saved / "fontopen-original.obj").read_bytes()),
            "fontopen_local_object_sha256": sha((saved / "fontopen.obj").read_bytes()),
            "fontopen_link_relevant_omf_sha256": link_relevant_omf_sha(saved / "fontopen.obj"),
            "fontopen_omf_code_sha256": sha(local_fontopen_code),
            "fontopen_omf_data_sha256": sha(local_fontopen_data),
            "version_member_position": order.index("version"),
            "version_original_object_sha256": sha((saved / "version-original.obj").read_bytes()),
            "version_local_object_sha256": sha((saved / "version.obj").read_bytes()),
            "version_link_relevant_omf_sha256": link_relevant_omf_sha(saved / "version.obj"),
            "version_data_sha256": sha(local_version),
            "grp_member_position": order.index("grp"),
            "grp_original_object_sha256": sha((saved / "grp-original.obj").read_bytes()),
            "grp_local_object_sha256": sha((saved / "grp.obj").read_bytes()),
            "grp_link_relevant_omf_sha256": link_relevant_omf_sha(saved / "grp.obj"),
            "grp_data_sha256": sha(local_grp),
            "grp_extdefs": list(extdef_names(saved / "grp.obj")),
            "file_state_member_position": order.index("fil"),
            "file_state_original_object_sha256": sha((saved / "fil-original.obj").read_bytes()),
            "file_state_local_object_sha256": sha((saved / "fil.obj").read_bytes()),
            "file_state_link_relevant_omf_sha256": link_relevant_omf_sha(saved / "fil.obj"),
            "file_state_data_sha256": sha(local_file_state),
            "file_state_bss_size": FILE_STATE_BSS_SIZE,
        }
...
def main() -> int:
    ...
    target_fontopen_code = payload[
        FONTOPEN_TARGET_CODE_OFFSET:FONTOPEN_TARGET_CODE_OFFSET + FONTOPEN_CODE_SIZE
    ]
    target_fontopen_data = payload[
        FONTOPEN_TARGET_DATA_OFFSET:FONTOPEN_TARGET_DATA_OFFSET + FONTOPEN_DATA_SIZE
    ]
    target_version_data = payload[
        VERSION_TARGET_DATA_OFFSET:VERSION_TARGET_DATA_OFFSET + VERSION_DATA_SIZE
    ]
    target_grp_data = payload[
        GRP_TARGET_DATA_OFFSET:GRP_TARGET_DATA_OFFSET + GRP_DATA_SIZE
    ]
    target_file_state_data = payload[
        FILE_STATE_TARGET_DATA_OFFSET:
        FILE_STATE_TARGET_DATA_OFFSET + FILE_STATE_DATA_SIZE
    ]
    ...
    if (len(target_fontopen_code), sha(target_fontopen_code)) != (
        FONTOPEN_CODE_SIZE, FONTOPEN_TARGET_CODE_SHA256
    ):
        raise RuntimeError("attested target FONTOPEN CODE changed")
    if (len(target_fontopen_data), sha(target_fontopen_data)) != (
        FONTOPEN_DATA_SIZE, FONTOPEN_TARGET_DATA_SHA256
    ):
        raise RuntimeError("attested target FONTOPEN DATA changed")
    if (len(target_version_data), sha(target_version_data)) != (
        VERSION_DATA_SIZE, VERSION_DATA_SHA256
    ):
        raise RuntimeError("attested target VERSION DATA changed")
    if payload[VERSION_TARGET_PADDING_OFFSET] != 0:
        raise RuntimeError("attested VERSION linker alignment changed")
    if (len(target_grp_data), sha(target_grp_data)) != (
        GRP_DATA_SIZE, GRP_DATA_SHA256
    ):
        raise RuntimeError("attested target GRP DATA changed")
    if payload[GRP_TARGET_PADDING_OFFSET] != 0:
        raise RuntimeError("attested GRP linker alignment changed")
    if (len(target_file_state_data), sha(target_file_state_data)) != (
        FILE_STATE_DATA_SIZE, FILE_STATE_DATA_SHA256
    ):
        raise RuntimeError("attested target FIL DATA changed")
    ...
    for source in (
        ...
        LOCAL_FONTOPEN, LOCAL_VERSION, LOCAL_GRP, LOCAL_FILE_STATE,
    ):
    ...
    if candidate[
        FONTOPEN_CANDIDATE_CODE_OFFSET:FONTOPEN_CANDIDATE_CODE_OFFSET + FONTOPEN_CODE_SIZE
    ] != target_fontopen_code:
        raise RuntimeError("local FONTOPEN linked CODE differs from target")
    if candidate[
        FONTOPEN_CANDIDATE_DATA_OFFSET:FONTOPEN_CANDIDATE_DATA_OFFSET + FONTOPEN_DATA_SIZE
    ] != target_fontopen_data:
        raise RuntimeError("local FONTOPEN linked DATA differs from target")
    if candidate[
        VERSION_CANDIDATE_DATA_OFFSET:VERSION_CANDIDATE_DATA_OFFSET + VERSION_DATA_SIZE
    ] != target_version_data:
        raise RuntimeError("local VERSION linked DATA differs from target")
    if candidate[VERSION_CANDIDATE_PADDING_OFFSET] != 0:
        raise RuntimeError("local VERSION linker alignment changed")
    if candidate[
        GRP_CANDIDATE_DATA_OFFSET:GRP_CANDIDATE_DATA_OFFSET + GRP_DATA_SIZE
    ] != target_grp_data:
        raise RuntimeError("local GRP linked DATA differs from target")
    if candidate[GRP_CANDIDATE_PADDING_OFFSET] != 0:
        raise RuntimeError("local GRP linker alignment changed")
    if candidate[
        FILE_STATE_CANDIDATE_DATA_OFFSET:
        FILE_STATE_CANDIDATE_DATA_OFFSET + FILE_STATE_DATA_SIZE
    ] != target_file_state_data:
        raise RuntimeError("local FIL linked DATA differs from target")
    differences = sum(a != b for a, b in zip(candidate, target_component))
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned MASTER file-state DATA/BSS replaces external FIL library member after local VERSION/GRP support; no authored or target exact claim",
        ...
        "target_fontopen_code_extent": "decoded ZUN.COM 0x136C..0x1383",
        "target_fontopen_code_sha256": FONTOPEN_TARGET_CODE_SHA256,
        "target_fontopen_data_extent": "decoded ZUN.COM _DATA 0x2212..0x2213",
        "target_fontopen_data_sha256": FONTOPEN_TARGET_DATA_SHA256,
        "target_version_data_extent": "decoded ZUN.COM _DATA 0x219A..0x21F6",
        "target_version_data_sha256": VERSION_DATA_SHA256,
        "target_version_padding": {"offset": "0x21F7", "value": 0},
        "target_grp_data_extent": "decoded ZUN.COM _DATA 0x21F8..0x2202",
        "target_grp_data_sha256": GRP_DATA_SHA256,
        "target_grp_padding": {"offset": "0x2203", "value": 0},
        "target_file_state_data_extent": "decoded ZUN.COM _DATA 0x220E..0x2211",
        "target_file_state_data_sha256": FILE_STATE_DATA_SHA256,
        "file_state_bss_layout": "candidate resident _BSS:19DA size 0x14; uninitialized and absent from COM file bytes",
        "input_sha256": inputs,
        ...
        "fontopen_code_raw_equal": True,
        "fontopen_data_raw_equal": True,
        "version_data_raw_equal": True,
        "grp_data_raw_equal": True,
        "version_grp_alignment_raw_equal": True,
        "file_state_data_raw_equal": True,
        "file_state_bss_layout_equal": True,
        "exact": differences == 0,
        "limit": "FIL is library-origin support. Only its four initialized DATA bytes have a target file extent; the 0x14 BSS owner is accepted only as OMF/MAP/link layout and has no invented target bytes. External CRT/libc, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
