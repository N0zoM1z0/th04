#!/usr/bin/env python3
"""Cold-compile maintained OP music_update_render_and_flip and relink OP_MUSIC_TEXT."""
...
RELOCATIONS = 804
SOURCE = ROOT / "src/op/music/music_update_render_and_flip.cpp"
BODY = ROOT / "src/op/music/music_update_render_and_flip.inl"
DEPENDENCIES = (
...
TARGET_FUNCTION_SHA = "dec941d0d2d279b96ec44aa34384b88478572adfa40691bdf3eff9546ad1acf5"
...
def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP music_update_render_and_flip target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("OP music_update_render_and_flip Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x1B04
        ...
        or int(row["caller_count"]) != 4
        or int(row["callee_count"]) != 4
    ):
        raise RuntimeError(f"OP music_update_render_and_flip Ghidra extent drift: {row!r}")

    if (
        body[:3] != bytes.fromhex("55 8b ec")
        or body[3] != 0xE8
        or START + 6 + int.from_bytes(body[4:6], "little", signed=True) != 0xBFA7
        or body[6:12] != bytes.fromhex("66 68 0f 00 ce 00")
        or body[12:17] != bytes.fromhex("9a d2 10 00 00")
        or body[17] != 0xE8
        or START + 20 + int.from_bytes(body[18:20], "little", signed=True) != 0xC04E
        or body[20:27] != bytes.fromhex("ba 7c 00 b0 00 ee ba")
        or body[27:29] != bytes.fromhex("a4 00")
        or body[29] != 0xA0
        or int.from_bytes(body[30:32], "little") != 0x3A7D
        or body[32:37] != bytes.fromhex("ee b0 01 2a 06")
        or int.from_bytes(body[37:39], "little") != 0x3A7D
        or body[39] != 0xA2
        or int.from_bytes(body[40:42], "little") != 0x3A7D
        or body[42:48] != bytes.fromhex("ba a6 00 ee 6a 01")
        or body[48] != 0x9A
        or int.from_bytes(body[49:51], "little") != 0x0CCE
        or int.from_bytes(body[51:53], "little") != 0x0DA1
        or body[53:] != bytes.fromhex("5d c3")
    ):
        raise RuntimeError("OP music_update_render_and_flip instruction topology drift")

    required = (
        "0A74:1867 idle  nopoly_b_put()",
        "0000:10D2       GRCG_SETCOLOR",
        "0A74:190E idle  polygons_update_and_render()",
        "0F34:3A7D idle  _music_page_accessed",
        "0DA1:0CCE       frame_delay_2(int)",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("candidate MAP no longer corroborates music update operands/calls")
    ...
    return {
        ...
        "caller_count": 4,
        "callee_count": 4,
        "near_calls": ["0xBFA7 nopoly_b_put", "0xC04E polygons_update_and_render"],
        "far_calls": ["0000:10D2 GRCG_SETCOLOR", "0DA1:0CCE frame_delay_2"],
        "page_global": "0F34:3A7D _music_page_accessed",
        "inline_ports": ["0x7C", "0xA4", "0xA6"],
    }
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void near music_update_render_and_flip(void)\n"
    end_marker = b"\n\n#if (GAME <= 3)\n#define cmt_bg_blit_planar"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("music_update_render_and_flip source anchors drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start)
    replacement = b'#include "src/op/music/music_update_render_and_flip.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/music/music_update_render_and_flip.cpp": sha_file(SOURCE),
        "src/op/music/music_update_render_and_flip.inl": sha_file(BODY),
    ...
    if "0A74:1B04 idle  music_update_render_and_flip()" not in map_text:
        raise RuntimeError("v489 music_update_render_and_flip MAP public drift")
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v610-music-update-standalone-{label}",
               "src/op/music/music_update_render_and_flip.cpp")
        ...
        target_as_unlinked = (
            body[:4] + bytes((0, 0))
            + body[6:13] + bytes((0, 0, 0, 0))
            + body[17:18] + bytes((0, 0))
            + body[20:30] + bytes((0, 0))
            + body[32:37] + bytes((0, 0))
            + body[39:40] + bytes((0, 0))
            + body[42:49] + bytes((0, 0, 0, 0))
            + body[53:]
        )
        if (
            not local_omf["valid"]
            ...
            or local_fixups != [(3, 49), (1, 40), (1, 37), (1, 30), (1, 18), (3, 13), (1, 4)]
        ):
            raise RuntimeError(f"{label}: standalone music_update_render_and_flip OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_music.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v610-music-update-group-{label}", "th04/op_music.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained music_update_render_and_flip changed grouped OP_MUSIC_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: linked OP music-update layout/body drift")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable_keys):
        raise RuntimeError("independent OP music_update_render_and_flip cold rounds differ")
    current_hashes = {
        "src/op/music/music_update_render_and_flip.cpp": sha_file(SOURCE),
        "src/op/music/music_update_render_and_flip.inl": sha_file(BODY),
        **{path: sha_file(ROOT / path) for path in DEPENDENCIES},
    ...
    if current_hashes != source_hashes:
        raise RuntimeError("maintained music_update_render_and_flip source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed music_update_render_and_flip maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "limit": "Decoded 55-byte function only; v489 remains surrounding build scaffold. No packed-file or whole-OP exactness.",
    }
