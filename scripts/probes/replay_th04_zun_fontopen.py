#!/usr/bin/env python3
"""Cold-replace ZUN's FONTFILE_OPEN/DOS_ROPEN library member after local file helpers."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local FONTOPEN changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local FONTOPEN changed MAP")
...
def main() -> int:
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned DOS_ROPEN/FONTFILE_OPEN code and file_sharingmode data replace one external library member after local file helpers; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "DOS_ROPEN/FONTFILE_OPEN and file_sharingmode are library-origin support, not authored ZUN progress. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
