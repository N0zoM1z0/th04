...
v678 reconstructs the 137-byte `polygon_build(...)` helper at payload
`0xBFC5` (`1A74:1885`) inside `OP_MUSIC_TEXT`. Target Ghidra
closes one contiguous near Pascal body with one caller and one callee. Target
operands and MAP evidence bind `polar(int,int,int)`, `CosTable8`,
`SinTable8`, the 4-bit center-Y subpixel shift, the 8-bit angle formula
`((i << 8) / point_count) + plus_angle`, per-point stores, and the final
copy of point 0 that closes the polygon.

Maintained natural source is `src/op/music/polygon_build.cpp`. TC86 emits
exactly 137 `OP_MUSIC_TEXT` bytes; masking four ordinary table/FAR-call
fixups on both streams leaves byte-identical instruction code. Focused A/B
cold replay raw-matches the complete function and the full 0x6A5-byte
`OP_MUSIC_TEXT` owner while preserving grouped OMF, complete OP EXE/MAP
identity, and all 804 ordered relocations. An independent focused recheck also
passes. The v679 OP aggregate passes 62/62 registered decoded slices raw-zero,
receipt SHA-256
`453c09aca5a08735cce654a390342c6e13c4c2aedffc5b2f48d11860f3c17166`.
No DIET-packed offset or whole-OP exactness is claimed.
...
v673 reconstructs the 131-byte `cfg_save()` helper at payload `0xA7F0`
(`1A74:00B0`) inside `OP_MAIN_TEXT`. Target Ghidra closes one
contiguous near body with three callers and four callees. Target operands and
MAP data bind `MIKO.CFG`, `FILE_APPEND`, `FILE_SEEK`, `FILE_WRITE`,
`FILE_CLOSE`, six resident option fields, a 6-byte options write, seek to
structure offset 9, a one-byte checksum write, and near return.

Maintained natural source is `src/op/config/cfg_save.cpp`. TC86 emits
exactly 131 `OP_MAIN_TEXT` bytes; masking eight ordinary data/FAR-call
fixups on both streams leaves byte-identical instruction code. Focused A/B
cold replay raw-matches the complete function and the full 0xD53-byte
`OP_MAIN_TEXT` owner while preserving grouped OMF, complete OP EXE/MAP
identity, and all 804 ordered relocations. The v674 OP aggregate passes
60/60 registered decoded slices raw-zero, receipt SHA-256
`4e4e4efd3696823256117ea94e6029d035687015c38fb2244157ff7ec39f5fd9`.
No DIET-packed offset or whole-OP exactness is claimed.
...
v666 reconstructs the 123-byte `start_game()` wrapper at payload
`0xA8F1` (`1A74:01B1`) inside `OP_MAIN_TEXT`. Target Ghidra
closes one contiguous near body with no direct callers and seven callees.
Target operands bind resident stage 0, configured lives/bombs copied into
credits, Reimu/stage ASCII initialization, cancellable playchar menu, demo
reset, CDG/config/gaiji cleanup, KAJA song fade with argument 10, shared
`game_exit`, and the resident debug flag selecting `main` versus `deb`
for `execl`.

Maintained natural source is `src/op/start/start_game.cpp`. Standalone TC86
output is exactly 123 `OP_MAIN_TEXT` bytes. The 14 ordinary fixups include
the expected +5 addend from `main` to `deb`; masking fixup fields on
both streams leaves byte-identical instruction code. Focused A/B cold replay
raw-matches the complete function and the full 0xD53-byte `OP_MAIN_TEXT`
owner while preserving grouped OMF, complete OP EXE/MAP identity, and all 804
ordered relocations. The v667 OP aggregate passes 57/57 registered decoded
slices raw-zero, receipt SHA-256
`d1f7bc4d49de74c6809542f18a9f0020664bd271693c5e6e792523b1390e80bd`.
No DIET-packed offset or whole-OP exactness is claimed.

## Latest verified cohort: OP menu-selection updater

v664 reconstructs the 110-byte `menu_sel_update_and_render(char,char)`
helper at payload `0xAE28` (`1A74:06E8`) inside `OP_MAIN_TEXT`.
Target Ghidra closes one contiguous near Pascal body with two callers and
three direct callees. Raw target operands and the pinned MAP bind the signed
one-byte `menu_sel` state, indirect near-Pascal renderer pointer,
inactive/active colors 1/8, signed wrap at 0/max, Extra-lock skip through
`extra_unlocked` and `in_option`, and the forced SE 1 playback.

Maintained natural source is `src/op/main/menu_sel_update.cpp`. Standalone
TC86 output is exactly 110 `OP_MAIN_TEXT` bytes; every pre-link delta is
confined to 16 ordinary data/indirect/near/FAR-call fixups. Focused A/B cold
replay raw-matches the complete function and the full 0xD53-byte
`OP_MAIN_TEXT` owner while preserving grouped OMF, complete OP EXE/MAP
identity, and all 804 ordered relocations. An independent focused recheck also
passes. The v665 OP aggregate passes 56/56 registered decoded slices raw-zero,
receipt SHA-256
`4cecd54a7748a3c67af0094cdcc55514fe573613e328f6cb21fa6afe16dd5855`.
No DIET-packed offset or whole-OP exactness is claimed.

## Latest verified cohort: OP Extra-start wrapper

v662 reconstructs the 93-byte `start_extra()` wrapper at payload `0xA96C`
(`1A74:022C`) inside `OP_MAIN_TEXT`. Target Ghidra closes one
contiguous near body with no direct callers and seven callees. Target operands
bind the resident Extra-stage setup (stage 6, 3 lives, 2 bombs, Reimu ASCII,
stage ASCII), the cancellable character menu, demo reset, CDG cleanup, config
save, gaiji restore, KAJA song fade with argument 10, shared `game_exit`,
and the two identical `main` filename arguments to `execl`.

Maintained natural source is `src/op/start/start_extra.cpp`. Standalone
TC86 output is exactly 93 `OP_MAIN_TEXT` bytes; every pre-link delta is
confined to 11 ordinary data/near/FAR-call fixups. Focused A/B cold replay
raw-matches the complete function and the full 0xD53-byte `OP_MAIN_TEXT`
owner while preserving grouped OMF, complete OP EXE/MAP identity, and all 804
ordered relocations. An independent focused recheck also passes. The v663 OP
aggregate passes 55/55 registered decoded slices raw-zero, receipt SHA-256
`e6c34cc34d82cfd5546edc4eaf975d79023c00e6bc285a3ddd44072c2309d9d3`.
No DIET-packed offset or whole-OP exactness is claimed.
...
v658 reconstructs the 86-byte `playchar_menu_put_initial()` helper at payload
`0xD6B2` (`1A74:2F72`) inside `OP_01_TEXT`. Target Ghidra closes
one contiguous near body with one caller and nine callees. Raw target operands
and the pinned MAP bind `PaletteTone=0`, `PALETTE_SHOW`,
`pi_load(0, "slb1.pi")`, page access/show writes, `pi_palette_apply(0)`,
`pi_put_8(0,0,0)`, `raise_bg_allocate_and_snap`, both playchar title
boxes, `pic_put`, `GRAPH_COPY_PAGE(0)`, and `PALETTE_BLACK_IN(1)`.

Maintained natural source is `src/op/menu/playchar_menu_initial.cpp`.
Standalone TC86 output is exactly 86 CODE bytes; every pre-link difference is
confined to 12 ordinary data/call fixups. Focused A/B cold replay raw-matches
the complete function and the full `0xAB3`-byte `OP_01_TEXT` producer,
while preserving grouped OMF, the complete OP EXE/MAP identity, and all 804
ordered relocations. The v659 complete OP aggregate passes 53/53 registered
decoded functions raw-zero, receipt SHA-256
`ee2b4c6eefeb5e406e5430ecbcf007299fe05b8fcff376466032d8f6e358f788`.
No DIET-packed offset or whole-OP exactness is claimed.
...
target review closes one contiguous near Pascal body with one caller and no
callees. Raw target and the pinned MAP bind `_script_p` at `0E53:3F48`,
`_script_param_number_default` at `0E53:3F94`, and `__ctype` at
`0E53:0B68`. The body reads three bytes, tests digit bit 2 through
`__ctype[c+1]`, computes one/two/three-digit decimal values, rolls
`script_p` back by 3/2/1/0 bytes, and terminates with Pascal `RET 4`.

Maintained natural source is `src/maine/cutscene/script_param_first.cpp`.
Standalone TC86 output is exactly 198 CODE bytes; all 13 pre-link differences
are ordinary 16-bit fixups, including the +1 addend on each `__ctype` lookup.
Focused A/B cold replay raw-matches the complete function and the full
`0xC3E`-byte `CUTSCENE_TEXT` producer while preserving grouped OMF, the
retained MAINE program image, and all 559 ordered relocations. An independent
focused recheck reproduces the same result. Two complete v650 MAINE aggregate
runs pass 37/37 registered decoded functions raw-zero; the primary receipt is
`.analysis/reconstruction/probes/v650-maine-script-param-first-aggregate-001/receipt.json`,
SHA-256 `b1240a448f47a76dea6365f877f2ca92e7424e355bb716d3762d996d9a4dee4a`.
This is decoded-function exactness only; no DIET-packed offset or whole-MAINE
exactness is claimed.

## Prior verified cohort: MAINE box background restore helper
...
review closes one contiguous near body with one caller and three callees. Raw
target and the pinned MAP bind cursor x/y state, the fast-forward flag, the
box animation path, conditional input wait, two direct page switches, and
`box_bg_put`; the function terminates with a near `RET`.

Maintained natural source is `src/maine/cutscene/cursor_advance.cpp`. Focused
A/B cold replay raw-matches all 84 function bytes and the complete
`0xC3E`-byte `CUTSCENE_TEXT` producer while preserving grouped OMF, the
retained MAINE program image, and all 559 ordered relocations. An independent
focused recheck reproduces the same result. The first full aggregate attempt
did not test the function: its replay rejected the nested private output
directory used by the aggregate wrapper. Commit `8289f36` fixes that replay
integration bug without changing the maintained function source. The repaired
v644 MAINE aggregate then passes 35/35 registered decoded functions raw-zero:
`.analysis/reconstruction/probes/v644-maine-cursor-advance-aggregate-recheck-002/receipt.json`,
SHA-256 `c446e0d301093817490558a05b41a020a781e02fb427151ad5738cb0fdace83c`.
This is decoded-function exactness only; no DIET-packed offset or whole-MAINE
exactness is claimed.

## Prior verified cohort: OP playchar picture darken helper
