#!/usr/bin/env python3
"""Cold-replace ZUN's FILE_CREATE library member after local DOS_PUTS2 support."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local FILE_CREATE changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local FILE_CREATE changed MAP")
...
def main() -> int:
    ...
    for source in (
        ...
        LOCAL_DOS_AXDX, LOCAL_DOS_PUTS2, LOCAL_FILE_CREATE,
    ):
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned FILE_CREATE support replaces external library member after local DOS_PUTS2 support; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "FILE_CREATE is library-origin support; its trailing module NOP is alignment outside the 0x3B-byte function owner. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
