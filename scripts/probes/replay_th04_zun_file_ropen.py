#!/usr/bin/env python3
"""Cold-replace ZUN's FILE_ROPEN library member after local FILE_CREATE support."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local FILE_ROPEN changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local FILE_ROPEN changed MAP")
...
def main() -> int:
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned FILE_ROPEN support replaces external library member after local FILE_CREATE support; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "FILE_ROPEN is library-origin support, not authored ZUN progress. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
