#!/usr/bin/env python3
"""Cold-replay maintained MAINE regist_menu and final SCORE EGC helpers."""
...
def overlay_source(work: Path) -> str:
    ...
    # Replace the final function first so earlier offsets remain irrelevant.
    ...
    egc_start_anchor = b"void near score_egc_start_copy(void)\n"
    egc_end_anchor = b'\n\n#include "src/maine/score/score_rect_copy.inl"\n'
    if data.count(egc_start_anchor) != 1 or data.count(egc_end_anchor) != 1:
        raise RuntimeError("score_egc_start_copy replacement anchors drift")
    egc_start = data.index(egc_start_anchor)
    egc_end = data.index(egc_end_anchor, egc_start)
    data = (
        data[:egc_start]
        + b'#include "src/maine/score/score_egc_start_copy.inl"'
        + data[egc_end:]
    )

    reg_start_anchor = b"void near regist_menu(void)\n"
    reg_end_anchor = b'\n\n\n#include "src/maine/score/score_egc_start_copy.inl"'
    if data.count(reg_start_anchor) != 1 or data.count(reg_end_anchor) != 1:
        raise RuntimeError("regist_menu replacement anchors drift")
    reg_start = data.index(reg_start_anchor)
    reg_end = data.index(reg_end_anchor, reg_start)
    data = (
        data[:reg_start]
        + b'#include "src/maine/score/regist_menu.inl"'
        + data[reg_end:]
    )
...
def main() -> int:
    ...
    closure = source_closure(ROOT, (
        "src/maine/score/regist_menu.inl",
        "src/maine/score/score_egc_start_copy.inl",
        "src/maine/score/score_rect_copy.inl",
    ))
    ...
    for label in ("a", "b"):
        ...
        obj.unlink()
        tcc(work, output, f"v720-score-tail-{label}", "th04/scoreall.cpp")
    ...
    receipt = {
        ...
        "claim_scope": "MAINE regist_menu + final SCORE EGC helpers maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        ...
        "compiler_shape": {
            "optimization_barrier": "empty inline bool returning false under TC86/MS-DOS; emits no target byte sequence",
            "keep_0": "relocatable zero-expression mechanism also independently used by TH05 EGC address-register code",
        },
        "builds": builds,
        "limit": (
            "Decoded 0x39C-byte regist_menu, 0x43-byte score EGC-start helper, and "
            "0x86-byte score rectangle-copy helper inside the existing exact 0xB30 SCORE_TEXT producer. "
    ...
    print(json.dumps({
        ...
        "regist_menu_sha256": REG_SHA,
        "egc_start_sha256": EGC_SHA,
