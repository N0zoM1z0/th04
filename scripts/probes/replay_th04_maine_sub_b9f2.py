#!/usr/bin/env python3
"""Cold-replay maintained natural C++ for MAINE sub_B9F2 and its switch table."""
...
SOURCE = ROOT / "src/maine/end/sub_B9F2.inl"
START = 0xB9F2
SIZE = 0x182
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "7f9efd60f9cff94f2925749ac9f6ac4d7c49c6cf20c153981754665916ebcdde"
AUX_START = NEXT
AUX_SIZE = 0x0D
AUX_NEXT = AUX_START + AUX_SIZE
TARGET_AUX_SHA = "21705111948a226c639a2d58dd385b280f8abd5e0890da5e8f410265ff2d304a"
...
def validate_target(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("MAINE sub_B9F2 target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("MAINE sub_B9F2 Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A05
        or int(row["entry_offset"], 0) != 0x19A2
        or int(row["body_min_linear"], 0) != 0x10000 + START
        or int(row["body_max_linear"], 0) != 0x10000 + NEXT - 1
        or int(row["body_addresses"]) != 0x148
        or int(row["body_span"]) != SIZE
        or row["contiguous"] != "false"
        or row["body_range_count"] != "2"
        or int(row["caller_count"]) != 1
        or int(row["callee_count"]) != 3
    ):
        raise RuntimeError(f"MAINE sub_B9F2 Ghidra extent drift: {row!r}")
    required = (
    ...
    if any(item not in map_text for item in required):
        raise RuntimeError("MAINE sub_B9F2 MAP evidence drift")
    if not body.startswith(bytes.fromhex("c8 08 00 00 c4 1e")):
        raise RuntimeError("MAINE sub_B9F2 prologue drift")
    if body[-2:] != bytes.fromhex("c9 c3"):
        raise RuntimeError("MAINE sub_B9F2 epilogue drift")
    return {
...
def overlay_source(work: Path) -> str:
    ...
    start_anchor = b"void near sub_B9F2(void)\n"
    end_anchor = b"\n\n#pragma codeseg\n"
    if data.count(start_anchor) != 1 or data.count(end_anchor) != 1:
        raise RuntimeError("sub_B9F2 replacement anchors drift")
    start = data.index(start_anchor)
    end = data.index(end_anchor, start)
    upstream.write_bytes(
        data[:start] + b'#include "src/maine/end/sub_B9F2.inl"' + data[end:]
    )
...
def main() -> int:
    ...
    closure = source_closure(ROOT, ("src/maine/end/sub_B9F2.inl",))
    ...
    if sha(aux) != TARGET_AUX_SHA or aux != bytes.fromhex("00 c8 19 d2 19 dc 19 e6 19 f0 19 fa 19"):
        raise RuntimeError("MAINE sub_B9F2 switch-table identity drift")
    ...
    if baseline.program_image[START:NEXT] != body:
        raise RuntimeError("v489 sub_B9F2 bytes differ from target")
    if baseline.program_image[AUX_START:AUX_NEXT] != aux:
        raise RuntimeError("v489 sub_B9F2 switch table differs from target")
    ...
    for label in ("a", "b"):
        ...
        obj.unlink()
        tcc(work, output, f"v714-sub-b9f2-{label}", "th04/gv.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained sub_B9F2 changed gv MAINE_01_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: cold MAINE sub_B9F2 link/layout drift")
        builds[label] = {
    ...
    if stable(builds["a"]) != stable(builds["b"]):
        raise RuntimeError("independent sub_B9F2 cold rounds differ")
    if any(prior.sha_file(ROOT / name) != digest for name, digest in source_hashes.items()):
        raise RuntimeError("maintained sub_B9F2 source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "MAINE sub_B9F2 maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        "target_restored_sha256": prior.TARGET_SHA,
        "source_sha256": source_hashes,
        "driver_sha256": prior.sha_file(Path(__file__).resolve()),
        "boundary": boundary,
        "auxiliary_switch_table": {
        ...
        "builds": builds,
        "limit": (
            "Decoded 386-byte sub_B9F2 body plus its adjacent 13-byte compiler switch table. The surrounding gv.cpp is pinned "
