...
`config/evidence.csv`, and `config/knowledge.csv` retain the former v1-v116
...
v99-v116 expand the reviewed denominator outside the completed MAIN_036 frontier.
...
the remaining word-aligned assembler contribution. v104 then recovers the 0x65 MAIN_01_TEXT `BB_TXT_LOAD`/`BB_TXT_FREE` prefix from pure C++, including the target far-pointer offset for the second file read. The same review returns `pointnums_add_yellow` to provisional because its 0x1A Ghidra body jumps into a shared tail at 0x23DBE that is also reached by `pointnums_add_white`. v105 then recovers the following 0x91-byte `mugetsu_fg_render` prefix from natural C++; its first cold build exposed the separate-public ownership hazard, fixed by changing the assembler top-level PUBLIC to EXTRN when TC4J becomes the producer. v106 then recovers the following 0x4B `gengetsu_bomb_inv_render` helper from pure C++, replacing the previously published semantic alias with maintained source. Its first probe was two bytes too long because a fixed `_SI` expression produced MOV+ADD; compiler-managed register allocation emits the target LEA. This closes the target-derived C/C++ contribution in MAIN_01_TEXT while leaving the independent scoreupd.asm owner separate. v107 then opens MAIN_033_TEXT with the 0x8C Mugetsu gather prefix. Fresh Ghidra correctly sees the first 0x15 helper but truncates `mugetsu_gather_intro` to 0x11; TASM/raw/reviewer proof expands it to 0x77, consisting of 0x67 code bytes plus contiguous 4-word compare and 4-word jump tables through 0x280BA. v108 then recovers the immediately following 0x6F `mugetsu_180BB` phase-state callback. Ghidra has no entry there; TASM/raw/owner-end proof plus two target function-pointer immediates close the boundary. The first source was five bytes too long due to early returns; an ordinary C++ shared `ret0` goto shape matches exactly. v109 then recovers the 0xCC `reimu_fg_render` BOSS_FG_TEXT suffix on its first pure-C++ focused build, reusing the existing Reimu trail alias and publishing the unchanged orb-render helper with a byte-preserving label. v110 replaces that alias with the exact 0x5C natural-C++ `reimu_orbs_render`, and v111 peels the preceding 0x47 `items_render` suffix. `_ES = SEG_PLANE_B` naturally emits the target VRAM segment setup, while delaying SI/DI register-local assignment until after the splash call preserves target instruction order. v112 then recovers the final 0x10B `BULLETS_RENDER` owner. Same-game exact compiler evidence identifies `#pragma option -G` as the source-level setting for the target explicit BP/SUB frame, and `_AX = bullet->patnum` preserves the target one-load/three-compare register reuse. This closes target-derived BOSS_FG_TEXT ownership to zero bytes. v113 then recovers the Ghidra-missed 0xC0 `yuuka6_bg_render` BOSS_BG_TEXT suffix on the first compilable pure-C++ source. TASM/raw plus the generated public and exact next-owner boundary close its function extent, and the remaining target-derived BOSS_BG_TEXT contribution drops from 0x7FB to 0x73B. v115 then corrects `sub_11ECB` from Ghidra's 0x2C body to the target-proved 0xCB `stage_state_init` MAIN_012_TEXT suffix. Pinned TASM/raw close at RET 0x21F95 and the next exact public at 0x21F96; the first pure-C++ source reproduces all scalar resets, nine storage clears, and gather-template initialization, reducing target-derived MAIN_012_TEXT ownership from 0x803 to 0x738. v116 then peels the preceding Ghidra-missed 0xB9 `elly_fg_render` suffix. Raw/TASM and the generated public close at RET 0x21ECA immediately before exact `stage_state_init`; a one-byte first-probe register-choice mismatch is solved by passing the still-live `_AX` for the no-damage top argument, reducing target-derived MAIN_012_TEXT ownership from 0x738 to 0x67F. The rolled-back v114 MAI_TEXT scroll-update probe is retained as negative knowledge: its best allowed source remained seven bytes long because TC4 could not simultaneously preserve the desired AX result and SUB condition flags under the explored compound-assignment forms. The discarded Mugetsu 0x2812A/0x2821E probe is retained as negative knowledge: `-a` restores both dense-switch metadata bytes and 33-word tables, but TC4J still canonicalizes two unique return gates and remains four bytes nonexact. Focused and 153-owner
aggregate cold replays preserve raw bytes, exact map contribution, ordered
relocation overlap, and deterministic TC86 OMF. The v116 boundary audit again finds all 434 MAIN TASM `PROC` starts represented in the ledger, while 81 provisional/high-risk MAIN candidates remain and boundary exploration is therefore not exhausted. For handoff, the next `MAIN_012_TEXT` suffix `sub_11DE6` is target-reviewed as one FAR 0x2C function that scans nine shot-level thresholds with `LOOP`, selects the player shot routine, calls `hud_power_put`, and RETFs; no source/unit is accepted yet, and the preliminary v117 candidate was removed before this handoff. Segment-edge owners remain
