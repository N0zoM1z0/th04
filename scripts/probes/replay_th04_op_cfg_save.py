...
def target_boundary(body: bytes, target_program: bytes, map_text: str) -> dict[str, object]:
    ...
    if (
        ...
        or int(row["entry_offset"], 0) != 0x00B0
        ...
        raise RuntimeError(f"OP cfg_save Ghidra extent drift: {row!r}")

    if (
        body[:4] != bytes.fromhex("c8 08 00 00")
        or body[4:9] != bytes.fromhex("1e 68 08 01 9a")
        or int.from_bytes(body[9:11], "little") != 0x089A
        or body[13:19] != bytes.fromhex("66 6a 00 6a 00 9a")
        or int.from_bytes(body[19:21], "little") != 0x0AB6
        or body[23:27] != bytes.fromhex("c4 1e 64 1a")
        or body[27:69] != bytes.fromhex(
            "26 8a 47 0f 88 46 f8 "
            "26 8a 47 3a 88 46 f9 "
            "26 8a 47 3b 88 46 fa "
            "26 8a 47 10 88 46 fb "
            "26 8a 47 18 88 46 fc "
            "26 8a 47 49 88 46 fd"
        )
        or body[69:77] != bytes.fromhex("16 8d 46 f8 50 6a 06 9a")
        or int.from_bytes(body[77:79], "little") != 0x0AF8
        or body[81:87] != bytes.fromhex("66 6a 09 6a 00 9a")
        or int.from_bytes(body[87:89], "little") != 0x0AB6
        or body[91:112] != bytes.fromhex(
            "8a 46 f8 02 46 f9 02 46 fa 02 46 fb 02 46 fc 02 46 fd 88 46 fe"
        )
        or body[112:120] != bytes.fromhex("16 8d 46 fe 50 6a 01 9a")
        or int.from_bytes(body[120:122], "little") != 0x0AF8
        or body[124] != 0x9A
        or int.from_bytes(body[125:127], "little") != 0x095A
...
def overlay_source(work: Path) -> str:
    ...
    upstream.write_bytes(
        data[:start] + b'#include "src/op/config/cfg_save.inl"' + data[end:]
    )
...
def main() -> int:
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"cfg-save-standalone-{label}", "src/op/config/cfg_save.cpp")
    ...
    receipt = {
        ...
        "builds": builds,
        "limit": "Decoded 131-byte cfg_save only; no packed-file or whole-OP exactness.",
    }
