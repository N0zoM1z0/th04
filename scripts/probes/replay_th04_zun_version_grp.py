#!/usr/bin/env python3
"""Cold-replace ZUN's VERSION and GRP pure-data library members after local FONTOPEN support."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local VERSION/GRP changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local VERSION/GRP changed MAP")
...
def main() -> int:
    ...
    for source in (
        ...
        LOCAL_FONTOPEN, LOCAL_VERSION, LOCAL_GRP,
    ):
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned MASTER version and graphics-state DATA replace two external pure-data library members after local FONTOPEN support; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "VERSION and GRP are library-origin pure-data support. The zero bytes following each owner are TLINK word-alignment gaps outside both DATA units. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
