#!/usr/bin/env python3
"""Cold-replace ZUN's FILE_SEEK/FILE_TELL library member after local FILE_WRITE support."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local FILE_SEEK changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local FILE_SEEK changed MAP")
...
def main() -> int:
    ...
    for source in (
        ...
        LOCAL_FILE_WRITE, LOCAL_FILE_SEEK,
    ):
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned FILE_SEEK and FILE_TELL support replace one external library member after local FILE_WRITE support; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "FILE_SEEK and FILE_TELL are library-origin support. The one-byte 0x90 between them is module alignment outside both function owners. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
