#!/usr/bin/env python3
"""Cold-compile maintained OP singleline and relink OP_SETUP_TEXT."""
...
SOURCE = ROOT / "src/op/setup/singleline.cpp"
BODY = ROOT / "src/op/setup/singleline.inl"
START = 0xB5E2
SIZE = 0x7A
NEXT = START + SIZE
LOCAL_START = START - PRODUCER_START
TARGET_FUNCTION_SHA = "7e790b27f8250a222656455638ab8dd2a102d8550b67a666dcbe6d28f6ebeb1d"


def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP singleline target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("OP singleline Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x0EA2
        ...
        raise RuntimeError(f"OP singleline Ghidra extent drift: {row!r}")
    ...
    if (
        ...
        or body[21:23] != bytes.fromhex("6a 20")
        or body[30:32] != bytes.fromhex("6a 05")
        or body[38:42] != bytes.fromhex("8d 45 10 50")
        or body[42:44] != bytes.fromhex("6a 06")
        or body[61:63] != bytes.fromhex("6a 01")
        or body[69:73] != bytes.fromhex("8d 45 10 50")
        or body[73:75] != bytes.fromhex("6a 03")
        or body[97:99] != bytes.fromhex("6a 08")
        or body[105:109] != bytes.fromhex("8d 45 10 50")
        or body[109:111] != bytes.fromhex("6a 07")
        or body[-6:] != bytes.fromhex("5f 5e c9 c2 04 00")
    ):
        raise RuntimeError("OP singleline instruction/operand topology drift")

    required = (
        "0F34:2B48 idle  _window",
        "0DA1:0968       egc_copy_rect_1_to_0_16(int,int,int,int)",
        "0000:2D5A       SUPER_PUT",
        "0A74:0EA2 idle  singleline(int,int)",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("OP singleline MAP target drift")
    ...
    return {
        ...
        "constants": {"MSWIN_W": 16, "MSWIN_H": 16, "copy_height": 32},
        "terminal": "RET 4",
    }
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void pascal near singleline(screen_x_t left, screen_y_t top)\n"
    end_marker = b"\n\nvoid pascal near rollup(screen_x_t left, screen_y_t top)\n"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("OP singleline source anchors drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start)
    replacement = b'#include "src/op/setup/singleline.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/setup/singleline.cpp": sha_file(SOURCE),
        "src/op/setup/singleline.inl": sha_file(BODY),
    }
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v634-singleline-standalone-{label}", "src/op/setup/singleline.cpp")
        standalone = [
            path for path in (work / "obj/th04").glob("*.obj")
            if path.name not in before
        ]
        if len(standalone) != 1:
            raise RuntimeError(f"{label}: expected one standalone singleline object")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: standalone OP singleline OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_setup.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v634-singleline-group-{label}", "th04/op_setup.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained singleline changed grouped OP_SETUP_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: linked OP singleline/layout drift")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable):
        raise RuntimeError("independent OP singleline cold rounds differ")
    if (
        sha_file(SOURCE) != source_hashes["src/op/setup/singleline.cpp"]
        or sha_file(BODY) != source_hashes["src/op/setup/singleline.inl"]
    ):
        raise RuntimeError("maintained OP singleline source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed singleline maintained-source cold-link replay",
        "artifact": "th04-op",
