...
Updated 2026-09-24. This is the current-state index, not an experiment log.
...
All four targets passed the 2026-09-24 preflight; OP's database passed fresh
read-only attestation this session. Target canonicality remains
...
The non-MAIN authored backlog is 138 pending acceptances, 15 provisional
boundaries, and 33 unresolved `target-derived-asm` candidate representations.
...
26-byte places dispatcher, 49-byte alphabet cursor renderer, and the
...
Eight MAINE SCORE/high-score functions bring MAINE to 21 / 2,067; eight exact OP
SCORE functions bring OP to 21 / 2,387. OP `scoredat_recreate` at payload
`0xC68C` is decoded-exact after two focused cold links and the 21-slice
aggregate. `scoredat_encode` at `0xC627` is physically reviewed and now has a
maintained natural-C++ source, but pinned standalone codegen is 111 bytes rather
