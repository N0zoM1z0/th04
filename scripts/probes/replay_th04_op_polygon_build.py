#!/usr/bin/env python3
"""Cold-compile maintained OP polygon_build and relink OP_MUSIC_TEXT."""
...
RELOCATIONS = 804
SOURCE = ROOT / "src/op/music/polygon_build.cpp"
BODY = ROOT / "src/op/music/polygon_build.inl"
DEPENDENCIES = (
...
TARGET_FUNCTION_SHA = "622247641568682d06e13d410c42d081f2f1e594d69bee5ea24147e67e51a7ea"
...
def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP polygon_build target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("OP polygon_build Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x1885
        ...
        raise RuntimeError(f"OP polygon_build Ghidra extent drift: {row!r}")

    far_calls = ((52, 0x01A8), (84, 0x01A8))
    if (
        body[:14] != bytes.fromhex("c8 04 00 00 56 57 8b 7e 0e c1 7e 0a 04 c7")
        or body[14:19] != bytes.fromhex("46 fe 00 00 eb")
        or int.from_bytes(body[50:52], "little") != 0x0648
        or int.from_bytes(body[82:84], "little") != 0x05C8
        or any(
            body[off] != 0x9A
            or int.from_bytes(body[off + 1:off + 3], "little") != target_off
            or int.from_bytes(body[off + 3:off + 5], "little") != 0x0DA1
            for off, target_off in far_calls
        )
        or body[20:36] != bytes.fromhex(
            "8b 46 fe c1 e0 08 99 f7 7e 06 02 46 04 88 46 fd"
        )
        or body[109:] != bytes.fromhex(
            "8b 5e fe c1 e3 02 8b 05 89 01 "
            "8b 5e fe c1 e3 02 8b 45 02 89 41 02 "
            "5f 5e c9 c2 0c 00"
        )
    ):
        raise RuntimeError("OP polygon_build instruction/operand topology drift")

    required = (
        "0A74:1885 idle  polygon_build(screen_point_t near*,int,space_changing_pixel_t,int,int,unsigned char)",
        "0DA1:01A8       polar(int,int,int)",
        "0F34:0648       _CosTable8",
        "0F34:05C8       _SinTable8",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("OP polygon_build MAP target drift")
    ...
    return {
        ...
        "sin_table": "0F34:05C8",
        "subpixel_shift": 4,
        "angle_formula": "((i << 8) / point_count) + plus_angle",
        "closes_polygon": True,
        "terminal": "RET 0x0C",
    }
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void pascal near polygon_build(\n"
    end_marker = b"\n}\n\n#define polygon_init"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("polygon_build replacement anchor drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start) + 2
    replacement = b'#include "src/op/music/polygon_build.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/music/polygon_build.cpp": sha_file(SOURCE),
        "src/op/music/polygon_build.inl": sha_file(BODY),
    ...
    if "0A74:1885 idle  polygon_build(screen_point_t near*,int,space_changing_pixel_t,int,int,unsigned char)" not in map_text:
        raise RuntimeError("v489 polygon_build MAP public drift")
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v678-polygon-build-standalone-{label}",
               "src/op/music/polygon_build.cpp")
        ...
        expected_fixups = [(3, 85), (1, 82), (3, 53), (1, 50)]
        ...
        if (
            ...
            raise RuntimeError(f"{label}: standalone polygon_build OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_music.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v678-polygon-build-group-{label}", "th04/op_music.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained polygon_build changed grouped OP_MUSIC_TEXT")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable_keys):
        raise RuntimeError("independent OP polygon_build cold rounds differ")
    current_hashes = {
        "src/op/music/polygon_build.cpp": sha_file(SOURCE),
        "src/op/music/polygon_build.inl": sha_file(BODY),
        **{path: sha_file(ROOT / path) for path in DEPENDENCIES},
    ...
    if current_hashes != source_hashes:
        raise RuntimeError("maintained OP polygon_build source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed polygon_build maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "limit": "Decoded 137-byte function only; v489 remains surrounding build scaffold. No packed-file or whole-OP exactness.",
    }
