#!/usr/bin/env python3
"""Cold-compile maintained OP rollup and relink OP_SETUP_TEXT."""
...
SOURCE = ROOT / "src/op/setup/rollup.cpp"
BODY = ROOT / "src/op/setup/rollup.inl"
START = 0xB65C
SIZE = 0x3C
NEXT = START + SIZE
LOCAL_START = START - PRODUCER_START
TARGET_FUNCTION_SHA = "86622caa2fa0cd5eaad8d868f67d1b6ed7b95d704209b4d7b5f2c6d381871aa1"


def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP rollup target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("OP rollup Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x0F1C
        ...
        raise RuntimeError(f"OP rollup Ghidra extent drift: {row!r}")

    if (
        body[:9] != bytes.fromhex("55 8b ec 56 57 8b 76 04 a1")
        or int.from_bytes(body[9:11], "little") != 0x2B4A
        or body[11:29] != bytes.fromhex(
            "c1 e0 04 05 f0 ff 03 f0 bf 01 00 eb 12 ff 76 06 56 e8"
        )
        or START + 31 + int.from_bytes(body[29:31], "little", signed=True) != 0xB519
        or body[31:34] != bytes.fromhex("6a 01 9a")
        or int.from_bytes(body[34:36], "little") != 0x002B
        or int.from_bytes(body[36:38], "little") != 0x0DA1
        or body[38:43] != bytes.fromhex("47 83 ee 08 a1")
        or int.from_bytes(body[43:45], "little") != 0x2B4A
        or body[45:] != bytes.fromhex(
            "03 c0 05 fe ff 3b c7 7f e2 5f 5e 5d c2 04 00"
        )
    ):
        raise RuntimeError("OP rollup instruction/operand topology drift")

    required = (
        "0F34:2B48 idle  _window",
        "0A74:0DD9 idle  window_rollup_put(int,int)",
        "0DA1:002B       frame_delay(int)",
        "0A74:0F1C idle  rollup(int,int)",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("OP rollup MAP target drift")
    ...
    return {
        ...
        "window_h_operand": "0x2B4A (_window+2)",
        "near_call_target": "0xB519 window_rollup_put",
        "far_call_target": "0DA1:002B frame_delay",
        "constants": {"MSWIN_H": 16, "DROP_FRAMES_PER_TILE": 2, "DROP_SPEED": 8},
        "terminal": "RET 4",
    }
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void pascal near rollup(screen_x_t left, screen_y_t top)\n"
    end_marker = b"\n\ninline void window_animate(\n"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("OP rollup source anchors drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start)
    replacement = b'#include "src/op/setup/rollup.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/setup/rollup.cpp": sha_file(SOURCE),
        "src/op/setup/rollup.inl": sha_file(BODY),
    }
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v612-rollup-standalone-{label}", "src/op/setup/rollup.cpp")
        standalone = [
            path for path in (work / "obj/th04").glob("*.obj")
            if path.name not in before
        ]
        if len(standalone) != 1:
            raise RuntimeError(f"{label}: expected one standalone rollup object")
        ...
        target_unlinked = (
            body[:9] + bytes((2, 0)) + body[11:29] + bytes((0, 0))
            + body[31:34] + bytes((0, 0, 0, 0))
            + body[38:43] + bytes((2, 0)) + body[45:]
        )
        if (
            not local_omf["valid"]
            or "TC86 Borland C++ 4.02" not in local_omf["translator_comments"]
            or local_code != target_unlinked
            or local_fixups != [(1, 43), (3, 34), (1, 29), (1, 9)]
        ):
            raise RuntimeError(f"{label}: standalone OP rollup OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_setup.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v612-rollup-group-{label}", "th04/op_setup.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained rollup changed grouped OP_SETUP_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: linked OP rollup/layout drift")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable):
        raise RuntimeError("independent OP rollup cold rounds differ")
    if (
        sha_file(SOURCE) != source_hashes["src/op/setup/rollup.cpp"]
        or sha_file(BODY) != source_hashes["src/op/setup/rollup.inl"]
    ):
        raise RuntimeError("maintained OP rollup source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed rollup maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "builds": builds,
        "limit": "Decoded 60-byte function only; no DIET-packed or whole-OP exactness.",
    }
