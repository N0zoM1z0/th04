...
def main() -> int:
 ...
 ap.add_argument("--op-source-dir",type=Path,required=True); ap.add_argument("--maine-source-dir",type=Path,required=True); ap.add_argument("--op-target-restored",type=Path,required=True); ap.add_argument("--maine-target-restored",type=Path,required=True); ap.add_argument("--output-dir",type=Path,required=True)
 ...
 for art in ("op","maine"):
  ...
  if sha(exe)!=BASE[art]["exe"] or sha(mp)!=BASE[art]["map"]: raise ValueError(f"{art}: baseline identity drift")
  ...
  if len(bs)!=BASE[art]["reloc_count"] or Counter(bs)!=Counter(ts) or sum(a!=b for a,b in zip(bs,ts))!=8: raise ValueError(f"{art}: baseline relocation frontier drift")
 ...
 for label in ("a","b"):
  ...
  for art in ("op","maine"):
   ...
   if changed!=BASE[art]["changed"]: raise ValueError(f"{label}/{art}: changed-index drift")
 ...
 receipt={"schema_version":1,"observed_utc":datetime.now(timezone.utc).isoformat(timespec="seconds"),"claim_scope":"TH04 OP/MAINE BGIMAGE hybrid authored C++ exact producer and cross-game provenance replay","source_sha256":sha(SOURCE),"header_sha256":sha(HEADER),"hmem_header_sha256":sha(HMEM),"producer":"TC86 -B generated symbolic ASM -> pinned TASM32 5.0","crossgame":provenance,"builds":builds,"observed_effect":"Tracked hybrid C++ produces exact 0xD0 BGIMAGE CODE without __emit__ or codestring. Independent TH05 OP/MAINE targets preserve the same non-address machine-code architecture and the same eight relative relocation sites. TCC -B is required to carry symbolic REP MOVSD into generated ASM; pinned TASM32 emits the target ascending FIXUPP order. Replacing only BGIMAGE preserves the complete OP/MAINE program images and makes both full MZ relocation tables target-index exact. The only remaining target-restored program-image difference in each artifact is the shared two-byte snd_load encoding.","limit":"The hybrid source uses symbolic low-level DS/ES stack operations, REP MOVSD, and an EVEN directive only where pure C++/intrinsic probes are compiler-negative and independent TH05 targets corroborate the same architecture. No emitted opcodes, codestrings, object patches, FIXUPP edits, or MZ relocation permutation are used."}
