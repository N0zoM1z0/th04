...
class DecodedAcceptanceTests(unittest.TestCase):
    ...
    def test_maine_graph_3_digit_backend_is_artifact_source_and_producer_bound(self) -> None:
        entries = deepcopy(self.entries)
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-graph-3-digit-v704")
        owner["source"] = "src/maine/end/not_graph3.inl"
        with self.assertRaisesRegex(ValueError, "missing or mismatched maintained source"):
            self.check(entries)
        ...
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-graph-3-digit-v704")
        owner["producer_size"] = "0x3F9"
        with self.assertRaisesRegex(ValueError, "graph-3-digit backend does not compile"):
            self.check(entries)

        self.assertEqual(
            acceptance.backend_command(
                "maine-graph-3-digit-v704",
                ROOT / ".analysis/reconstruction/probes/test",
            )[1],
    ...
    def test_maine_sub_b81d_backend_is_artifact_source_and_producer_bound(self) -> None:
        entries = deepcopy(self.entries)
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-sub-b81d-v711")
        owner["source"] = "src/maine/end/not_sub_b81d.inl"
        with self.assertRaisesRegex(ValueError, "missing or mismatched maintained source"):
            self.check(entries)
        ...
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-sub-b81d-v711")
        owner["producer_offset"] = "0xB788"
        with self.assertRaisesRegex(ValueError, "sub-B81D backend does not compile"):
            self.check(entries)
        ...
        self.assertEqual(
            ...
            "scripts/probes/replay_th04_maine_sub_b81d.py",
        )
    ...
    def test_maine_main_backend_is_artifact_source_and_producer_bound(self) -> None:
        entries = deepcopy(self.entries)
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-main-v697")
        owner["source"] = "src/maine/end/not_main.inl"
        with self.assertRaisesRegex(ValueError, "missing or mismatched maintained source"):
            self.check(entries)
        ...
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-main-v697")
        owner["producer_offset"] = "0xA05A"
        with self.assertRaisesRegex(ValueError, "MAINE main backend does not compile"):
            self.check(entries)
        ...
        self.assertEqual(
            ...
            "scripts/probes/replay_th04_maine_main.py",
        )

    def test_maine_pic_put_both_masked_backend_is_artifact_source_and_producer_bound(self) -> None:
        entries = deepcopy(self.entries)
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-pic-put-both-masked-v691")
        owner["source"] = "src/maine/cutscene/pic_copy_to_other.cpp"
        with self.assertRaisesRegex(ValueError, "missing or mismatched maintained source"):
            self.check(entries)
        ...
        owner = next(row for row in entries
                     if row["replay_backend"] == "maine-pic-put-both-masked-v691")
        owner["producer_offset"] = "0xA293"
        with self.assertRaisesRegex(ValueError, "masked-picture backend does not compile"):
            self.check(entries)

        self.assertEqual(
            acceptance.backend_command(
                "maine-pic-put-both-masked-v691",
                ROOT / ".analysis/reconstruction/probes/test",
            )[1],
    ...
    def test_op_setup_submenus_backend_is_artifact_source_and_producer_bound(self) -> None:
        entries = deepcopy(self.entries)
        owner = next(row for row in entries
                     if row["replay_backend"] == "op-setup-submenus-v745")
        owner["source"] = "src/op/setup/not_setup_submenu.inl"
        with self.assertRaisesRegex(ValueError, "missing or mismatched maintained source"):
            self.check(entries)
        ...
        owner = next(row for row in entries
                     if row["replay_backend"] == "op-setup-submenus-v745")
        owner["producer_size"] = "0x5A5"
        with self.assertRaisesRegex(ValueError, "OP setup-submenus backend does not compile"):
            self.check(entries)

        self.assertEqual(
            acceptance.backend_command(
                "op-setup-submenus-v745",
                ROOT / ".analysis/reconstruction/probes/test",
            )[1],
            "scripts/probes/replay_th04_op_setup_submenus.py",
        )

    def test_zun_memchk_natural_backend_is_artifact_source_and_producer_bound(self) -> None:
        entries = deepcopy(self.entries)
        owner = next(row for row in entries
                     if row["replay_backend"] == "zun-memchk-natural-v773")
        owner["source"] = "src/zun/memchk/not_main.cpp"
        with self.assertRaisesRegex(ValueError, "missing or mismatched maintained source"):
            self.check(entries)
        ...
        owner = next(row for row in entries
                     if row["replay_backend"] == "zun-memchk-natural-v773")
        owner["producer_size"] = "0x0FE1"
        with self.assertRaisesRegex(ValueError, "ZUN MEMCHK natural backend does not compile"):
            self.check(entries)

        self.assertEqual(
            acceptance.backend_command(
                "zun-memchk-natural-v773",
                ROOT / ".analysis/reconstruction/probes/test",
            )[1],
            "scripts/probes/replay_th04_zun_memchk_natural.py",
        )
    ...
    def test_op_main_remaining_backend_is_artifact_source_and_producer_bound(self) -> None:
        entries = deepcopy(self.entries)
        owner = next(row for row in entries
                     if row["replay_backend"] == "op-main-remaining-v742")
        owner["source"] = "src/op/main/not_op_main.inl"
        with self.assertRaisesRegex(ValueError, "missing or mismatched maintained source"):
            self.check(entries)
        ...
        owner = next(row for row in entries
                     if row["replay_backend"] == "op-main-remaining-v742")
        owner["producer_size"] = "0xD52"
        with self.assertRaisesRegex(ValueError, "OP main remaining backend does not compile"):
            self.check(entries)

        self.assertEqual(
            acceptance.backend_command(
                "op-main-remaining-v742",
                ROOT / ".analysis/reconstruction/probes/test",
            )[1],
            "scripts/probes/replay_th04_op_main_remaining.py",
        )
