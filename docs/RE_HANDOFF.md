...
| Gameplay init/loop | DEMO_TEXT load 0xAD03..0xAECF / 0xAB88..0xAD02 | Init misses one metadata byte; maintained loop now compiles to 372/379 bytes with the target 25-byte interval branch. The frame-counter sequence and two `EB 00` jumps remain open. [Probe](reconstruction/TH04_DEMO_GAMEPLAY_TERNARY_V248.md) |
