#!/usr/bin/env python3
"""Cold-compile maintained OP cmt_put and relink OP_MUSIC_TEXT."""
...
RELOCATIONS = 804
SOURCE = ROOT / "src/op/music/cmt_put.cpp"
BODY = ROOT / "src/op/music/cmt_put.inl"
START = 0xC2C4
SIZE = 0x4A
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "a7c2fbd8e2e6a445190ef5798fd6fdff173ab6725992be9ded2e3d0f67db3b31"
...
def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP cmt_put target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("OP cmt_put Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x1B84
        ...
        raise RuntimeError(f"OP cmt_put Ghidra extent drift: {row!r}")

    if (
        body[:4] != bytes.fromhex("55 8b ec 56")
        or body[4:17] != bytes.fromhex("66 68 40 00 40 01 6a 07 1e 68 92 3a 9a")
        or int.from_bytes(body[17:19], "little") != 0x04A4
        or int.from_bytes(body[19:21], "little") != 0x0DA1
        or body[21:33] != bytes.fromhex("be 01 00 eb 28 8b de 6b db 28 80 bf")
        or int.from_bytes(body[33:35], "little") != 0x3A92
        or body[35:57] != bytes.fromhex("3b 74 1b 68 40 01 8d 44 04 c1 e0 04 50 6a 07 1e 8b c6 6b c0 28 05")
        or int.from_bytes(body[57:59], "little") != 0x3A92
        or body[59] != 0x50
        or body[60] != 0x9A
        or int.from_bytes(body[61:63], "little") != 0x04A4
        or int.from_bytes(body[63:65], "little") != 0x0DA1
        or body[65:] != bytes.fromhex("46 83 fe 14 7c d3 5e 5d c3")
    ):
        raise RuntimeError("OP cmt_put instruction/operand topology drift")

    if (
        "0F34:3A92 idle  _cmt" not in map_text
        or "0DA1:04A4       GRAPH_PUTSA_FX" not in map_text
        or "0A74:1B84 idle  cmt_put()" not in map_text
    ):
        raise RuntimeError("candidate MAP no longer corroborates cmt_put operands/call")
    ...
    return {
        ...
        "graph_call_target": "0DA1:04A4 GRAPH_PUTSA_FX",
        "cmt_line_size": 40,
        "line_count": 20,
        "skip_prefix": ";",
        "title": [320, 64, 7],
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void near cmt_put(void)\n"
    end_marker = b"\n\nvoid near cmt_fadein_both_animate(void)\n"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("cmt_put source anchors drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start)
    replacement = b'#include "src/op/music/cmt_put.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/music/cmt_put.cpp": sha_file(SOURCE),
        "src/op/music/cmt_put.inl": sha_file(BODY),
    }
    ...
    if "0A74:1B84 idle  cmt_put()" not in map_text:
        raise RuntimeError("v489 cmt_put MAP public drift")
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v618-cmt-put-standalone-{label}",
               "src/op/music/cmt_put.cpp")
        ...
        target_as_unlinked = bytes.fromhex(
            "55 8b ec 56 66 68 40 00 40 01 6a 07 1e 68 00 00 "
            "9a 00 00 00 00 be 01 00 eb 28 8b de 6b db 28 80 bf "
            "00 00 3b 74 1b 68 40 01 8d 44 04 c1 e0 04 50 6a 07 "
            "1e 8b c6 6b c0 28 05 00 00 50 9a 00 00 00 00 46 "
            "83 fe 14 7c d3 5e 5d c3"
        )
        if (
            not local_omf["valid"]
            or "TC86 Borland C++ 4.02" not in local_omf["translator_comments"]
            or local_code != target_as_unlinked
            or local_fixups != [(3, 61), (1, 57), (1, 33), (3, 17), (1, 14)]
        ):
            raise RuntimeError(f"{label}: standalone cmt_put OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_music.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v618-cmt-put-group-{label}", "th04/op_music.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained cmt_put changed grouped OP_MUSIC_TEXT")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable_keys):
        raise RuntimeError("independent OP cmt_put cold rounds differ")
    if sha_file(SOURCE) != source_hashes["src/op/music/cmt_put.cpp"] or sha_file(BODY) != source_hashes["src/op/music/cmt_put.inl"]:
        raise RuntimeError("maintained cmt_put source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed cmt_put maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "limit": "Decoded 74-byte function only; v489 remains surrounding build scaffold. No packed-file or whole-OP exactness.",
    }
