...
decoded-exact functions / 1,031 source-owner bytes per artifact. MAINE has 41
decoded-exact functions / 3,662 exact source-owner bytes plus two
...
`box_1_to_0_masked` and 52-byte nonexact `egc_start_copy`, for 4,037 decoded
source-owner bytes; OP has
64 decoded-exact functions / 5,840 exact source-owner bytes, with two separate
codec candidates (274 bytes), `snd_se_update` (76 bytes), `SND_SE_PLAY`
(57 bytes), and `nopoly_B_put` (30 bytes) still source-present and
nonexact, for 6,277 decoded source-owner bytes total.
...
v647/v648 then close the independent 175-byte `box_bg_put` leaf. Target-first
review binds the background buffer, all four VRAM planes, complete loop
geometry, and near RET before codegen work. Natural maintained C++ is exactly
175 CODE bytes modulo eight ordinary pointer fixups; focused A/B replay
raw-matches the full function and `CUTSCENE_TEXT` producer. The v648 MAINE
aggregate passes 36/36 decoded slices with all 559 ordered relocations
preserved. This exact result is separate from the adjacent v645 EGC load-order
gap and does not weaken that negative conclusion.
v649/v650 then close the 198-byte `script_param_read_number_first` leaf.
Target-first review binds the script pointer, default value, `__ctype` digit
classification, 3/2/1/0-byte rollback behavior, decimal arithmetic, and Pascal
cleanup. Natural maintained C++ is exactly 198 CODE bytes modulo 13 ordinary
16-bit fixups, including the +1 `__ctype` addends; focused replay raw-matches
the full function and `CUTSCENE_TEXT` producer. Two v650 aggregate runs pass
37/37 decoded slices with all 559 ordered relocations preserved. This is
function-level decoded exactness only, not a whole-MAINE or packed-file claim.
v651/v652 then close the 117-byte `pic_copy_to_other` helper. Target-first
review binds `egc_start_copy`, `EGC_OFF`, the B-plane pointer, 320x200
geometry, page 0-to-1 copy direction, and Pascal cleanup. Natural maintained
C++ is exactly 117 CODE bytes modulo four ordinary OMF fixups; focused replay
raw-matches the function and `CUTSCENE_TEXT` producer, and the v652 MAINE
...
v656/v657 then close MAINE `game_init_main`. Target-first review separates
the 77-byte FAR Pascal function from the 78-byte `initmain.obj` SHARED owner.
Natural maintained C++ emits the complete unlinked instruction shape modulo
ten ordinary fixups and one expected same-segment FAR-call opcode difference;
TLINK converts `vram_planes_set` to the target NOP/PUSH-CS/CALL-rel16 footprint.
The grouped owner, full EXE/MAP, and all 559 relocations remain baseline-exact.
The v657 MAINE aggregate passes 40/40 decoded slices raw-zero.
v658/v659 then close OP `playchar_menu_put_initial`. Target-first review
binds the palette reset, `slb1.pi` load/render sequence, page control,
raise-background snapshot, both playchar title boxes, character picture, page
copy, black-in, and near RET. Natural maintained C++ is exactly 86 CODE bytes
modulo 12 ordinary fixups; focused replay raw-matches the function and full
`OP_01_TEXT` producer, and the v659 OP aggregate passes 53/53 decoded slices
with all 804 ordered relocations preserved.
v660/v661 then close the adjacent 98-byte `shottype_menu_put_initial`.
Target-first review binds character-dependent CDG slot 40/41 selection, the
two expanded GRCG shadow rectangles, direct GRCG-off, the shot-type title box,
and selected title render. Natural maintained C++ is exactly 98 CODE bytes
modulo eight ordinary fixups; focused replay raw-matches the function and full
`OP_01_TEXT` producer, and the v661 OP aggregate passes 54/54 decoded slices
with all 804 ordered relocations preserved.
v662/v663 then close the 93-byte `start_extra` wrapper. Target-first
review binds the Extra-stage resident fields, cancellable playchar menu, demo
reset, cleanup/save/gaiji chain, KAJA fade, shared `game_exit`, and `execl`
of `main`. Natural maintained C++ is exactly 93 CODE bytes modulo 11
ordinary fixups; focused replay raw-matches the complete function and 0xD53-byte
`OP_MAIN_TEXT` owner, and the v663 OP aggregate passes 55/55 decoded slices
with all 804 ordered relocations preserved.
v664/v665 then close the 110-byte `menu_sel_update_and_render` helper.
Target-first review binds signed one-byte menu wrapping, the indirect renderer
pointer, Extra-lock skip, active/inactive colors, and forced selection SE.
Natural maintained C++ is exactly 110 CODE bytes modulo 16 ordinary fixups;
...
with all 804 ordered relocations preserved.
v668/v669 then close the 126-byte `cfg_save_exit` helper. Target-first
review binds the 10-byte zero template, compiler-generated `F_SCOPY@`
initializer, `MIKO.CFG`, master.lib file calls, six resident option bytes,
checksum, and full 10-byte write. Natural `cfg_t cfg = { 0 }` C++ emits
the target copy-helper shape without low-level forcing; focused replay raw-matches
...
v671/v672 then close the 129-byte `game_init_op` wrapper. Target-first
review binds memory allocation, same-segment `vram_planes_set` call
optimization, page clearing, buffer/input/text/EGC/joystick setup, optional
packfile start, and `BGM_INIT(1024)`. Natural shared C++ is exactly 129
bytes modulo 15 ordinary fixups and the one TLINK-optimized call opcode;
...
granted.
v676/v677 then close the 119-byte `setup_menu` wrapper. Target-first
review binds the complete tone/BFNT/page/PI/palette/BGM/SE/cleanup sequence
across thirteen unique callees. Natural maintained C++ is exactly 119 CODE
bytes modulo nineteen ordinary fixups; focused replay raw-matches the complete
function and 0x5A6-byte `OP_SETUP_TEXT` owner, and the v677 OP aggregate
passes 61/61 decoded slices with all 804 ordered relocations preserved.
v678/v679 then close the 137-byte `polygon_build` helper. Target-first
review binds `polar`, `CosTable8`/`SinTable8`, the 4-bit center-Y
shift, 8-bit point-angle calculation, point-array stores, and polygon closure.
Natural maintained C++ is exactly 137 CODE bytes modulo four ordinary table/
FAR-call fixups; focused replay raw-matches the complete function and 0x6A5-byte
`OP_MUSIC_TEXT` owner, and the v679 OP aggregate passes 62/62 decoded slices
with all 804 ordered relocations preserved.
v680/v681 then close the 164-byte `cfg_load` helper. Target-first review
binds `MIKO.CFG`, the three file calls, 10-byte cfg load, resident-segment
far-pointer restoration, six option copies, and range correction. Natural
maintained C++ is exactly 164 CODE bytes modulo eleven ordinary data/FAR-call
fixups; focused replay raw-matches the complete function and 0xD53-byte
`OP_MAIN_TEXT` owner, and the v681 OP aggregate passes 63/63 decoded slices
with all 804 ordered relocations preserved.
v682/v683 then close the 156-byte MAINE `hiscore_scoredat_save` wrapper.
Target-first review binds rank/playchar section selection, `GENSOU.SCR`,
196-byte section geometry, the current-section write, and the ten-section
decode/encode/rewrite loop. Natural maintained C++ raw-matches the complete
function while preserving the entire grouped `SCORE_TEXT` owner; the v683
MAINE aggregate passes 41/41 decoded slices with all 559 ordered relocations
preserved. Exact credit remains local to the wrapper; both score codecs remain
source-present/nonexact.
v684/v685 then close the 179-byte OP `playchar_titles_put` renderer.
Target-first review binds Reimu/Marisa title geometry, selected/unselected
colors, the two far-string entries per character, four `GRAPH_PUTSA_FX`
calls, and `1-sel` other-character selection. Natural maintained C++ is
exactly 179 CODE bytes modulo eight ordinary table/FAR-call fixups; focused
replay raw-matches the complete function and 0xAB3-byte `OP_01_TEXT` owner,
and the v685 OP aggregate passes 64/64 decoded slices with all 804 ordered
...
decoded-function totals are 64/64 OP and 41/41 MAINE. The next unit is not
