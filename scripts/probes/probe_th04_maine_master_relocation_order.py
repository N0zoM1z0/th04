...
TARGET_RESTORED_SHA='7e6b78861613cdd06d72e2f8584268444642b31abcc35d686c0ed1f4cc668f70'
...
def main()->int:
 ap=argparse.ArgumentParser(description=__doc__); ap.add_argument('--source-dir',type=Path,required=True); ap.add_argument('--target-restored',type=Path,required=True); ap.add_argument('--output-dir',type=Path); a=ap.parse_args(); src=a.source_dir.resolve(); target=a.target_restored.resolve(); out=outdir(a.output_dir)
 for rel,h in BASE.items():
  p=src/rel
  if not p.is_file() or sha(p)!=h: raise ValueError(f'v401 baseline drift: {rel}')
 if not target.is_file() or sha(target)!=TARGET_RESTORED_SHA: raise ValueError('target-restored MZ identity drift')
 target_sites=sites(target); baseline=order_metrics(sites(src/'bin/th04/maine.exe'),target_sites)
 ...
 for lab in ('a','b'):
  ...
  exe=work/'bin/th04/maine.exe'; mp=work/'obj/th04/maine.map'; payload=compare('th04-maine',work); valid_payload(lab,payload); metrics=order_metrics(sites(exe),target_sites)
  ...
  builds[lab]={'exe_sha256':sha(exe),'map_sha256':sha(mp),'payload_comparison':payload,'relocation_order':metrics}
 for key in ('exe_sha256','map_sha256','relocation_order'):
 ...
 rec={'schema_version':1,'claim_scope':'TH04 MAINE natural master-object link-order diagnostic; no historical relocation-order or packed-file exactness claim','baseline_identity':BASE,'target_restored_sha256':TARGET_RESTORED_SHA,'baseline_relocation_order':baseline,'baseline_payload_comparison':base_payload,'object_order_before':['mainem.obj','vsorig.obj','mainemdata.obj','mainemmid.obj','mainemtail.obj'],'object_order_after':['mainem.obj','mainemmid.obj','mainemtail.obj','vsorig.obj','mainemdata.obj'],'builds':builds,'observed_effect':'Without editing any relocation table bytes, moving the two code-tail objects before the historical VS/data contributions reduces the DIET-restored-view ordered mismatch from 13 indices (42..54) to only indices 48..49 while preserving the two-byte snd_load-only payload residual and target-equal relocation multiset.','limit':'The DIET -RA restored order equals stub application order, not independently observed historical TLINK order. The final two swapped entries are therefore diagnostic ambiguity, not an acceptance failure to repair by table permutation.'}; rp=out/'receipt.json'; rp.write_text(json.dumps(rec,indent=2)+'\n'); print(json.dumps({'receipt':str(rp),'receipt_sha256':sha(rp),'baseline_mismatches':13,'reordered_mismatches':2,'candidate_sha256':NEW_EXE},sort_keys=True)); return 0
