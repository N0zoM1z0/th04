...
| MAINE.EXE | 72 | 23 / 42 / 7 | 17 | 55 | Four SCORE_TEXT owners accepted; inspect adjacent place-row owner |
| ZUN.COM | 13 | 13 / 0 / 0 | 0 | 11, plus 2 blocked | `_main` natural CALL shape; ZUNINIT/MEMCHK source authority |

The non-MAIN authored backlog is 147 pending acceptances, 15 provisional
boundaries, and 39 unresolved `target-derived-asm` candidate representations.
...
and 172-byte name cursor renderer;
...
Four MAINE SCORE functions bring MAINE to 17 / 1,703; the OP stage renderer
...
- v548 accepts the next MAINE SCORE_TEXT cursor renderer at `1A05:2615`, payload `0xC665..0xC710`, 172 bytes ending `RET 6`. The standalone object differs only in the two-byte near-call displacement with an attested OMF fixup; grouped SCORE_TEXT and both cold-linked functions are raw-zero. The full candidate EXE/MAP and 559 ordered relocations remain stable. The decoded wrapper now covers 17 MAINE slices; packed-file and whole-TU exactness remain open. Next review `place_row_put` at `0xC711`. See the [SCORE note](reconstruction/op-maine/TH04_MAINE_SCORE_CPP_V479.md#v548-maintained-name-cursor-renderer).
