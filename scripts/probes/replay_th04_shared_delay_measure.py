#!/usr/bin/env python3
"""Cold-relink OP and MAINE with the maintained shared delay-until-measure TU.
...
SOURCE = ROOT / "src/shared/sound/delay_until_measure.cpp"
...
ARTIFACTS = {
    "op": (0xDD80, 0x31, 804, "v228-op-target-roundtrip/a/restored.bin",
    ...
    "maine": (0xD046, 0x31, 559, "v228-maine-target-roundtrip/a/restored.bin",
...
def contribution(path: Path) -> tuple[int, int, str]:
    pattern = re.compile(
        ...
        r"\bS=SHARED\b.*\bM=th04/snd_dlym\.cpp(?:\s|$)", re.I,
    )
    matches = []
    ...
    if len(matches) != 1:
        raise RuntimeError(f"expected one delay-until-measure MAP contribution: {path}")
    return matches[0]
...
def main() -> int:
    ...
    for name, (start, size, reloc_count, target_rel, target_sha, baseline_sha) in ARTIFACTS.items():
        ...
        if len(targets[name].program_image[start:start + size]) != size:
            raise RuntimeError(f"{name}: truncated target delay-until-measure function")
    ...
    for label in ("a", "b"):
        ...
        for name, (start, size, reloc_count, _, _, baseline_sha) in ARTIFACTS.items():
            ...
            shutil.copy2(SOURCE, work / "th04/snd_dlym.cpp")
            ...
            obj = work / "obj/th04/snd_dlym.obj"
            obj.unlink()
            command = ["wine", str(RUNNER), "-e", "-x", "tcc", "-c", "-I.",
                       "-O", "-b-", "-3", "-Z", "-d", "-DGAME=4", "-ml",
                       "-nobj/th04/", "th04/snd_dlym.cpp"]
            ...
            if differences or candidate.program_image != baselines[name].program_image:
                raise RuntimeError(f"{label}/{name}: delay-until-measure function or aggregate program differs")
            if sha(exe.read_bytes()) != baseline_sha:
                raise RuntimeError(f"{label}/{name}: aggregate EXE identity drift")
            builds[label][name] = {
                "source_object_link_relevant_sha256": link_relevant_omf_sha(obj),
    ...
    receipt = {
        ...
        "claim_scope": "artifact-local decoded delay-until-measure function raw replay",
