#!/usr/bin/env python3
"""Cold-replay maintained natural C++ for MAINE graph_3_digit_put."""
...
SOURCE = ROOT / "src/maine/end/graph_3_digit_put.inl"
START = 0xB787
SIZE = 0x96
NEXT = START + SIZE
...
PRODUCER_SIZE = 0x3FA
LOCAL_START = 0
TARGET_FUNCTION_SHA = "1c0b9d4366722fbb762b4750f7ec537c4b7f37847c1c37d30ef3d855b5ba38e6"
...
def validate_target(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("MAINE graph_3_digit_put target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("MAINE graph_3_digit_put Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A05
        or int(row["entry_offset"], 0) != 0x1737
        ...
        raise RuntimeError(f"MAINE graph_3_digit_put Ghidra extent drift: {row!r}")
    required = (
        "0A05:1737 03FA C=CODE   S=MAINE_01_TEXT  G=GROUP_01 M=th04/gv.cpp",
        "0A05:1737       graph_3_digit_put(int,int,unsigned int)",
        "0A05:17CD       sub_b81d()",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("MAINE graph_3_digit_put MAP evidence drift")
    if not body.startswith(bytes.fromhex("c8 06 00 00 56")):
        raise RuntimeError("MAINE graph_3_digit_put prologue drift")
    if body[-5:] != bytes.fromhex("5e c9 c2 06 00"):
        raise RuntimeError("MAINE graph_3_digit_put epilogue drift")
    return {
...
def overlay_source(work: Path) -> str:
    ...
    start_anchor = b"void pascal near graph_3_digit_put(\n"
    end_anchor = b"\n\nvoid near sub_B81D(void)"
    if data.count(start_anchor) != 1 or data.count(end_anchor) != 1:
        raise RuntimeError("graph_3_digit_put replacement anchors drift")
    start = data.index(start_anchor)
    end = data.index(end_anchor, start)
    upstream.write_bytes(
        data[:start] + b'#include "src/maine/end/graph_3_digit_put.inl"' + data[end:]
    )
...
def main() -> int:
    ...
    closure = source_closure(ROOT, ("src/maine/end/graph_3_digit_put.inl",))
    ...
    if baseline.program_image[START:NEXT] != body:
        raise RuntimeError("v489 graph_3_digit_put bytes differ from target")
    ...
    for label in ("a", "b"):
        ...
        obj.unlink()
        tcc(work, output, f"v704-graph-3-digit-{label}", "th04/gv.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained graph_3_digit_put changed gv MAINE_01_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: cold MAINE graph_3_digit_put link/layout drift")
        builds[label] = {
    ...
    if stable(builds["a"]) != stable(builds["b"]):
        raise RuntimeError("independent graph_3_digit_put cold rounds differ")
    if any(prior.sha_file(ROOT / name) != digest for name, digest in source_hashes.items()):
        raise RuntimeError("maintained graph_3_digit_put source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "MAINE graph_3_digit_put maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        ...
        "builds": builds,
        "limit": (
            "Decoded 150-byte graph_3_digit_put only. The surrounding gv.cpp is pinned "
