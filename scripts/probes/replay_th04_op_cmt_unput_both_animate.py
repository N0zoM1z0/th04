#!/usr/bin/env python3
"""Cold-compile maintained OP cmt_unput_both_animate and relink OP_MUSIC_TEXT."""
...
RELOCATIONS = 804
SOURCE = ROOT / "src/op/music/cmt_unput_both_animate.cpp"
BODY = ROOT / "src/op/music/cmt_unput_both_animate.inl"
START = 0xC33F
SIZE = 0x30
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "5337c81b6db988be1e1c5cee4723cc916ebc22599d5292d0d6f718db0623eea0"
...
def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP cmt_unput_both_animate target identity drift")
    import csv
    ...
    if len(rows) != 1:
        raise RuntimeError("OP cmt_unput_both_animate Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x1BFF
        ...
        raise RuntimeError(f"OP cmt_unput_both_animate Ghidra extent drift: {row!r}")
    if (
        body[:9] != bytes.fromhex("55 8b ec c7 06 3c 0a 02 00")
        or body[9:21] != bytes.fromhex("66 68 40 00 40 01 66 68 40 01 40 01")
        or body[21:26] != bytes.fromhex("9a e8 0a a1 0d")
        or body[26] != 0xE8
        or START + 29 + int.from_bytes(body[27:29], "little", signed=True) != 0xC244
        or body[29:41] != bytes.fromhex("66 68 40 00 40 01 66 68 40 01 40 01")
        or body[41:46] != bytes.fromhex("9a e8 0a a1 0d")
        or body[46:] != bytes.fromhex("5d c3")
    ):
        raise RuntimeError("OP cmt_unput_both_animate instruction topology drift")
    if (
        "0F34:0A3C       _graph_putsa_fx_func" not in map_text
        or "0DA1:0AE8       BGIMAGE_PUT_RECT_16" not in map_text
        or "0A74:1B04 idle  music_update_render_and_flip()" not in map_text
    ):
        raise RuntimeError("candidate MAP no longer corroborates cmt_unput operands/calls")
    return {
        ...
        "fx_value": 2,
        "rect": [320, 64, 320, 320],
        "far_call_target": "0DA1:0AE8 BGIMAGE_PUT_RECT_16",
        "near_call_target": "0xC244 music_update_render_and_flip",
    }
...
def overlay_source(work: Path) -> str:
    ...
    old = (
        b"void near cmt_unput_both_animate(void)\n"
        b"{\n"
        b"\t// ZUN bloat: Not related to unblitting.\n"
        b"\tgraph_putsa_fx_func = FX_WEIGHT_BOLD;\n"
        b"\n"
        b"\tcmt_unput();\n"
        b"\tmusic_update_render_and_flip();\n"
        b"\tcmt_unput();\n"
        b"}"
    )
    replacement = b'#include "src/op/music/cmt_unput_both_animate.inl"'
    if data.count(old) != 1:
        raise RuntimeError("cmt_unput_both_animate replacement anchor drift")
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/music/cmt_unput_both_animate.cpp": sha_file(SOURCE),
        "src/op/music/cmt_unput_both_animate.inl": sha_file(BODY),
    }
    ...
    if "0A74:1BFF idle  cmt_unput_both_animate()" not in map_text:
        raise RuntimeError("v489 cmt_unput_both_animate MAP public drift")
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v598-cmt-unput-standalone-{label}",
               "src/op/music/cmt_unput_both_animate.cpp")
        ...
        target_as_unlinked = (
            body[:5] + bytes((0, 0)) + body[7:22] + bytes((0, 0, 0, 0))
            + body[26:27] + bytes((0, 0)) + body[29:42]
            + bytes((0, 0, 0, 0)) + body[46:]
        )
        if (
            not local_omf["valid"]
            or "TC86 Borland C++ 4.02" not in local_omf["translator_comments"]
            or local_code != target_as_unlinked
            or local_fixups != [(3, 42), (1, 27), (3, 22), (1, 5)]
        ):
            raise RuntimeError(f"{label}: standalone cmt_unput_both_animate OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_music.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v598-cmt-unput-group-{label}", "th04/op_music.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained cmt_unput_both_animate changed grouped OP_MUSIC_TEXT")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable_keys):
        raise RuntimeError("independent OP cmt_unput_both_animate cold rounds differ")
    if sha_file(SOURCE) != source_hashes["src/op/music/cmt_unput_both_animate.cpp"] or sha_file(BODY) != source_hashes["src/op/music/cmt_unput_both_animate.inl"]:
        raise RuntimeError("maintained cmt_unput_both_animate source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed cmt_unput_both_animate maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "limit": "Decoded 48-byte function only; v489 remains surrounding build scaffold. No packed-file or whole-OP exactness.",
    }
