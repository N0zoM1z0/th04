#!/usr/bin/env python3
"""Cold-compile maintained OP raise_bg_free and relink OP_01_TEXT."""
...
RELOCATIONS = 804
SOURCE = ROOT / "src/op/menu/raise_bg_free.cpp"
BODY = ROOT / "src/op/menu/raise_bg_free.inl"
START = 0xD1F3
SIZE = 0x17
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "2e10c4e9a17f4826d3fd0866150e6ec27cbeb07c860128940fa95fed908b08df"
...
def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP raise_bg_free target identity drift")
    nd = shutil.which("ndisasm")
    if nd is None:
        raise RuntimeError("ndisasm unavailable")
    rows = disassemble(nd, body, START)
    if (
        [str(row["mnemonic"]) for row in rows]
        != ["push", "mov", "push", "call", "push", "call", "pop", "ret"]
        ...
        raise RuntimeError(f"OP raise_bg_free decode drift: {rows!r}")
    if (
        body[3:5] != b"\xff\x36"
        or int.from_bytes(body[5:7], "little") != 0x3F7C
        or body[7] != 0x9A
        or int.from_bytes(body[8:10], "little") != 0x2856
        or int.from_bytes(body[10:12], "little") != 0
        or body[12:14] != b"\xff\x36"
        or int.from_bytes(body[14:16], "little") != 0x3F80
        or body[16] != 0x9A
        or int.from_bytes(body[17:19], "little") != 0x2856
        or int.from_bytes(body[19:21], "little") != 0
        or body[21:] != b"\x5d\xc3"
    ):
        raise RuntimeError("OP raise_bg_free target operands drift")
    if (
        "0F34:3F7A idle  _raise_bg" not in map_text
        or "0000:2856       HMEM_FREE" not in map_text
    ):
        raise RuntimeError("candidate map no longer corroborates raise_bg/HMEM_FREE")
    return {
        ...
        "data_map_target": "0F34:3F7A _raise_bg",
...
def overlay_source(work: Path) -> str:
    ...
    old = (
        b"void near raise_bg_free(void)\n"
        b"{\n"
        b"\tHMem<dots8_t>::free(raise_bg[PLAYCHAR_REIMU]);\n"
        b"\tHMem<dots8_t>::free(raise_bg[PLAYCHAR_MARISA]);\n"
        b"}"
    )
    replacement = b'#include "src/op/menu/raise_bg_free.inl"'
    if data.count(old) != 1:
        raise RuntimeError("raise_bg_free replacement anchor drift")
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/menu/raise_bg_free.cpp": sha_file(SOURCE),
        "src/op/menu/raise_bg_free.inl": sha_file(BODY),
        "src/shared/memory/hmem.hpp": sha_file(ROOT / "src/shared/memory/hmem.hpp"),
    }
    ...
    if "0A74:2AB3 idle  raise_bg_free()" not in map_text:
        raise RuntimeError("v489 raise_bg_free MAP public drift")
    ...
    for label in ("a", "b"):
        ...
        shutil.copy2(BODY, dst / BODY.name)
        hmem_source = ROOT / "src/shared/memory/hmem.hpp"
        hmem_dest = work / "src/shared/memory/hmem.hpp"
        hmem_dest.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(hmem_source, hmem_dest)

        before = {path.name for path in (work / "obj/th04").glob("*.obj")}
        tcc_op(work, output, f"v588-raise-bg-free-standalone-{label}",
               "src/op/menu/raise_bg_free.cpp")
        ...
        target_as_unlinked = (
            body[:5] + b"\x02\x00" + body[7:8] + b"\0\0\0\0"
            + body[12:14] + b"\x06\x00" + body[16:17] + b"\0\0\0\0"
            + body[21:]
        )
        if (
            not local_omf["valid"]
            ...
            or local_fixups != [(3, 17), (1, 14), (3, 8), (1, 5)]
        ):
            raise RuntimeError(f"{label}: standalone raise_bg_free OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/m_char.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v588-raise-bg-free-group-{label}", "th04/m_char.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained raise_bg_free changed grouped OP_01_TEXT")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable_keys):
        raise RuntimeError("independent OP raise_bg_free cold rounds differ")
    if (
        sha_file(SOURCE) != source_hashes["src/op/menu/raise_bg_free.cpp"]
        or sha_file(BODY) != source_hashes["src/op/menu/raise_bg_free.inl"]
        or sha_file(ROOT / "src/shared/memory/hmem.hpp")
        != source_hashes["src/shared/memory/hmem.hpp"]
    ):
        raise RuntimeError("maintained raise_bg_free source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed raise_bg_free maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "limit": "Decoded 23-byte function only; v489 remains surrounding build scaffold. No packed-file or whole-OP exactness.",
    }
