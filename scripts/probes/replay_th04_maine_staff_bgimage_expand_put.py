#!/usr/bin/env python3
"""Cold-replay maintained natural C++ for MAINE staffroll_bgimage_expand_put."""
...
SOURCE = ROOT / "src/maine/end/staff_bgimage_expand_put.inl"
START = 0xB25B
SIZE = 0x36
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "378435b972c228aa4a986cea65a0f2f33277c50b8e35df06d0565c740e0cda55"
...
def validate_target(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("MAINE staffroll_bgimage_expand_put target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("MAINE staff background helper Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A05
        or int(row["entry_offset"], 0) != 0x120B
        ...
        raise RuntimeError(f"MAINE staff background helper Ghidra extent drift: {row!r}")
    required = (
        "0A05:0E80 08B7 C=CODE   S=MAINE_01_TEXT  G=GROUP_01 M=th04/staffall.cpp",
        "0A05:120B idle  staffroll_bgimage_expand_put(int,int,int,int,int)",
        "0CC7:0A86       BGIMAGE_PUT_RECT_16",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("MAINE staff background helper MAP evidence drift")
    if not body.startswith(bytes.fromhex("55 8b ec 56 8b 76 04 8b c6 99")):
        raise RuntimeError("MAINE staff background helper prologue/register shape drift")
    if body[-5:] != bytes.fromhex("5e 5d c2 0a 00"):
        raise RuntimeError("MAINE staff background helper epilogue drift")
    return {
...
def overlay_source(work: Path) -> str:
    ...
    start_anchor = (
        b"void pascal near staffroll_bgimage_expand_put(\n"
        b"\tscreen_x_t left, screen_y_t top, pixel_t w, pixel_t h, int distance\n"
        b")\n{"
    )
    end_anchor = b"\n}\n#pragma codeseg\n\nextern void (near pascal *near dissolve_put_func)"
    if data.count(start_anchor) != 1 or data.count(end_anchor) != 1:
        raise RuntimeError("staff background helper replacement anchors drift")
    start = data.index(start_anchor)
    end = data.index(end_anchor, start) + 2
    upstream.write_bytes(
        data[:start] + b'#include "src/maine/end/staff_bgimage_expand_put.inl"' + data[end:]
    )
...
def main() -> int:
    ...
    closure = source_closure(ROOT, ("src/maine/end/staff_bgimage_expand_put.inl",))
    ...
    if baseline.program_image[START:NEXT] != body:
        raise RuntimeError("v489 staff background helper bytes differ from target")
    ...
    for label in ("a", "b"):
        ...
        obj.unlink()
        tcc(work, output, f"v700-staff-bg-helper-{label}", "th04/staffall.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: cold MAINE staff background helper link/layout drift")
        builds[label] = {
    ...
    if stable(builds["a"]) != stable(builds["b"]):
        raise RuntimeError("independent staff background helper cold rounds differ")
    if any(prior.sha_file(ROOT / name) != digest for name, digest in source_hashes.items()):
        raise RuntimeError("maintained staff background helper source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "MAINE staffroll_bgimage_expand_put maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        ...
        "builds": builds,
        "limit": (
            "Decoded 54-byte helper only. The surrounding staffall.cpp is pinned replay "
