#!/usr/bin/env python3
"""Cold-replay maintained natural C++ for MAINE skill_apply_and_graph_percentage_put."""
...
SOURCE = ROOT / "src/maine/end/skill_apply_and_graph_percentage_put.inl"
START = 0xB886
SIZE = 0xF5
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "ef23bcc95623c6b19f15f0b6d04abc5d8cadc25ad00770b0ebafca0fa037f043"
...
def validate_target(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("MAINE skill_apply_and_graph_percentage_put target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("MAINE skill_apply_and_graph_percentage_put Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A05
        or int(row["entry_offset"], 0) != 0x1836
        ...
        raise RuntimeError(f"MAINE skill_apply_and_graph_percentage_put Ghidra extent drift: {row!r}")
    required = (
        ...
        "0A05:192B       graph_fraction_of_million_put(int,int,unsigned long)",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("MAINE skill_apply_and_graph_percentage_put MAP evidence drift")
    if not body.startswith(bytes.fromhex("c8 06 00 00 56 57")):
        raise RuntimeError("MAINE skill percentage prologue drift")
    if body[-6:] != bytes.fromhex("5f 5e c9 c2 08 00"):
        raise RuntimeError("MAINE skill percentage epilogue drift")
    return {
...
def overlay_source(work: Path) -> str:
    ...
    start_anchor = b"void pascal near skill_apply_and_graph_percentage_put(\n"
    end_anchor = b"\n\nvoid pascal near graph_fraction_of_million_put(\n"
    if data.count(start_anchor) != 1 or data.count(end_anchor) != 1:
        raise RuntimeError("skill_apply_and_graph_percentage_put replacement anchors drift")
    start = data.index(start_anchor)
    end = data.index(end_anchor, start)
    upstream.write_bytes(
        data[:start] + b'#include "src/maine/end/skill_apply_and_graph_percentage_put.inl"' + data[end:]
    )
...
def main() -> int:
    ...
    closure = source_closure(ROOT, ("src/maine/end/skill_apply_and_graph_percentage_put.inl",))
    ...
    if baseline.program_image[START:NEXT] != body:
        raise RuntimeError("v489 skill_apply_and_graph_percentage_put bytes differ from target")
    ...
    for label in ("a", "b"):
        ...
        obj.unlink()
        tcc(work, output, f"v706-skill-percentage-{label}", "th04/gv.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained skill_apply_and_graph_percentage_put changed gv MAINE_01_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: cold MAINE skill_apply_and_graph_percentage_put link/layout drift")
        builds[label] = {
    ...
    if stable(builds["a"]) != stable(builds["b"]):
        raise RuntimeError("independent skill_apply_and_graph_percentage_put cold rounds differ")
    if any(prior.sha_file(ROOT / name) != digest for name, digest in source_hashes.items()):
        raise RuntimeError("maintained skill_apply_and_graph_percentage_put source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "MAINE skill_apply_and_graph_percentage_put maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        ...
        "builds": builds,
        "limit": (
            "Decoded 245-byte skill_apply_and_graph_percentage_put only. The surrounding gv.cpp is pinned "
