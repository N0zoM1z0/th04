#!/usr/bin/env python3
"""Cold-replay maintained natural C++ for MAINE box_bg_allocate_and_snap."""
...
SOURCE = ROOT / "src/maine/cutscene/box_bg_snap.inl"
START = 0xA4AE
SIZE = 0xD1
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "7aa6bf58223e039a617bc05aa08d2475c3b37638117a2b9473b058541656a926"
...
def validate_target(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("MAINE box_bg_allocate_and_snap target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("MAINE box_bg_allocate_and_snap Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A05
        or int(row["entry_offset"], 0) != 0x045E
        ...
        raise RuntimeError(f"MAINE box_bg_allocate_and_snap Ghidra extent drift: {row!r}")
    if "0A05:045E idle  box_bg_allocate_and_snap()" not in map_text:
        raise RuntimeError("MAINE box_bg_allocate_and_snap MAP entry drift")
    if "0A05:052F idle  box_bg_free()" not in map_text:
        raise RuntimeError("MAINE following box_bg_free MAP entry drift")
    if not body.startswith(b"\xC8\x08\x00\x00\x56\x57") or body[-4:] != b"\x5F\x5E\xC9\xC3":
        raise RuntimeError("MAINE box_bg_allocate_and_snap prologue/epilogue drift")
    return {
        ...
        "caller_count": 1,
        "callee_count": 2,
        "allocation_bytes": 0x3C00,
        "terminal": "pop di; pop si; leave; ret",
        "next_function": "1A05:052F box_bg_free",
    }
...
def overlay_source(work: Path) -> str:
    ...
    start_anchor = b"void near box_bg_allocate_and_snap(void)\n{"
    end_anchor = b"\n}\n\nvoid near box_bg_free(void)"
    if data.count(start_anchor) != 1 or data.count(end_anchor) != 1:
        raise RuntimeError("box_bg_allocate_and_snap replacement anchors are not unique")
    start = data.index(start_anchor)
    end = data.index(end_anchor, start) + 2
    upstream.write_bytes(data[:start] + b'#include "src/maine/cutscene/box_bg_snap.inl"' + data[end:])
...
def main() -> int:
    ...
    closure = source_closure(ROOT, ("src/maine/cutscene/box_bg_snap.inl",))
    ...
    if baseline.program_image[START:NEXT] != body:
        raise RuntimeError("v489 box_bg_allocate_and_snap bytes differ from target")
    ...
    for label in ("a", "b"):
        ...
        group_obj.unlink()
        tcc(work, output, f"v689-box-bg-snap-group-{label}", "th04/cutscene.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained box_bg_allocate_and_snap changed grouped CUTSCENE_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: cold MAINE box_bg_allocate_and_snap link/layout drift")
        builds[label] = {
    ...
    if stable(builds["a"]) != stable(builds["b"]):
        raise RuntimeError("independent MAINE box_bg_allocate_and_snap cold rounds differ")
    if any(prior.sha_file(ROOT / name) != digest for name, digest in source_hashes.items()):
        raise RuntimeError("maintained box_bg_allocate_and_snap source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "MAINE target-reviewed box_bg_allocate_and_snap and maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        ...
        "builds": builds,
        "limit": (
            "Decoded 209-byte function only; no packed-file or whole-MAINE exactness. "
