#!/usr/bin/env python3
"""Cold-relink OP and MAINE with the maintained shared PI load TU.
...
SOURCE = ROOT / "src/shared/formats/pi_load.cpp"
...
ARTIFACTS = {
    "op": (0xDAFD, 0x46, 804, "v228-op-target-roundtrip/a/restored.bin",
    ...
    "maine": (0xCD65, 0x46, 559, "v228-maine-target-roundtrip/a/restored.bin",
...
def contribution(path: Path) -> tuple[int, int, str]:
    pattern = re.compile(
        ...
        r"\bS=SHARED\b.*\bM=th03/pi_load\.cpp(?:\s|$)", re.I,
    )
    matches = []
    ...
    if len(matches) != 1:
        raise RuntimeError(f"expected one PI load MAP contribution: {path}")
    return matches[0]
...
def main() -> int:
    ...
    for name, (start, size, reloc_count, target_rel, target_sha, baseline_sha) in ARTIFACTS.items():
        ...
        if len(targets[name].program_image[start:start + size]) != size:
            raise RuntimeError(f"{name}: truncated target PI load producer")
    ...
    for label in ("a", "b"):
        ...
        for name, (start, size, reloc_count, _, _, baseline_sha) in ARTIFACTS.items():
            ...
            shutil.copy2(SOURCE, work / "th03/pi_load.cpp")
            ...
            obj = work / "obj/th03/pi_load.obj"
            obj.unlink()
            command = ["wine", str(RUNNER), "-e", "-x", "tcc", "-c", "-I.",
                       "-O", "-b-", "-3", "-Z", "-d", "-DGAME=4", "-ml",
                       "-nobj/th03/", "th03/pi_load.cpp"]
            ...
            if differences or candidate.program_image != baselines[name].program_image:
                raise RuntimeError(f"{label}/{name}: PI load producer or aggregate program differs")
    ...
    receipt = {
        ...
        "claim_scope": "artifact-local decoded PI load producer raw replay",
