...
- The latest complete native aggregate is
  `gpt-web-cfg-impl-aggregate-001`: all 253 selected MAIN owners pass two
...
- The TH04 product tree now uses local runtime, graphics, platform, GRCG,
  ...
  geometry, item-overflow, midboss-state, scroll-state, sound-implementation, sound-measure, and config-loader interfaces. The
  remaining compatibility boundary is **5 forwarders, 6 include sites, and 6
...
1. **Finish standalone source closure.** Follow the
   ...
   beginning with `th04/formats/map.hpp` and
   `th04/main/bullet/laser_t.hpp`. Run a complete default aggregate after each
...
- `gpt-web-cfg-impl-aggregate-001`
