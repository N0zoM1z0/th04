...
53 decoded-exact functions / 4,431 exact source-owner bytes, with two separate
codec candidates (274 bytes) plus `snd_se_update` (76 bytes) still
source-present and nonexact, for 4,781 decoded source-owner bytes total.
...
decoded-function totals are 53/53 OP and 40/40 MAINE. The next unit is not
