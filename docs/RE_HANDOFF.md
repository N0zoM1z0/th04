...
- The latest complete native aggregate is
  `gpt-web-hud-metrics-aggregate-001`: all 253 selected MAIN owners pass two
...
- The TH04 product tree now uses local runtime, graphics, platform, GRCG,
  ...
  PI, tile-format, BB, enemy-size, maintained sprite-pattern, polar, subpixel, bullet-add implementation, and HUD
  geometry interfaces. The remaining compatibility boundary is **11 forwarders,
  12 include sites, and 12 product files**, with no missing, unused, invalid, or
...
1. **Finish standalone source closure.** Follow the
   ...
   beginning with `th02/main/item/shared.hpp` and
   `th02/main/midboss/midboss.hpp`. Run a complete default aggregate after each
...
- `gpt-web-hud-metrics-aggregate-001`
