#!/usr/bin/env python3
"""Cold-compile maintained OP playchar_title_box_put and relink OP_01_TEXT."""
...
RELOCATIONS = 804
SOURCE = ROOT / "src/op/menu/playchar_title_box_put.cpp"
BODY = ROOT / "src/op/menu/playchar_title_box_put.inl"
START = 0xD338
SIZE = 0x6A
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "95663762bc4414804d9e05bfe151fc2730c6f53055744634ccc9e3ca801cb559"
...
def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP playchar_title_box_put target identity drift")
    import csv
    ...
    if len(rows) != 1:
        raise RuntimeError("OP playchar_title_box_put Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x2BF8
        ...
        raise RuntimeError(f"OP playchar_title_box_put Ghidra extent drift: {row!r}")
    far_calls = (
        (36, 0x10D2, 0x0000),
        (60, 0x0FF0, 0x0000),
        (71, 0x10D2, 0x0000),
    ...
    if (
        body[:30] != bytes.fromhex(
            "55 8b ec 56 57 8b 46 04 0b c0 74 07 3d 01 00 74 07 eb "
            "08 be 50 00 eb 03 be 70 01 bf 38 01"
        )
        or body[30:36] != bytes.fromhex("66 68 01 00 c0 00")
        or any(
            body[off] != 0x9A
        ...
        or body[41:60] != bytes.fromhex(
            "8d 44 08 50 8d 45 08 50 8d 84 c8 00 50 8d 45 48 50 6a 08"
        )
        or body[65:71] != bytes.fromhex("66 68 02 00 c0 00")
        or body[76:89] != bytes.fromhex("56 57 8d 84 c0 00 50 8d 45 40 50 6a 08")
        or body[94:] != bytes.fromhex("ba 7c 00 b0 00 ee 5f 5e 5d c2 02 00")
    ):
        raise RuntimeError("OP playchar_title_box_put instruction/operand topology drift")
    required = (
        "0A74:2BF8 idle  playchar_title_box_put(int)",
        "0000:10D2       GRCG_SETCOLOR",
        "0000:0FF0       GRCG_ROUND_BOXFILL",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("OP playchar_title_box_put MAP target drift")
    return {
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void near pascal playchar_title_box_put(int playchar)\n"
    end_marker = b"\n\ninline void pic_put_for(\n"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("playchar_title_box_put replacement anchor drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start)
    replacement = b'#include "src/op/menu/playchar_title_box_put.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/menu/playchar_title_box_put.cpp": sha_file(SOURCE),
        "src/op/menu/playchar_title_box_put.inl": sha_file(BODY),
    ...
    if "0A74:2BF8 idle  playchar_title_box_put(int)" not in map_text:
        raise RuntimeError("v489 playchar_title_box_put MAP public drift")
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v638-playchar-title-box-put-standalone-{label}",
               "src/op/menu/playchar_title_box_put.cpp")
        ...
        expected_fixups = [(3, 90), (3, 72), (3, 61), (3, 37)]
        ...
        if (
            ...
            raise RuntimeError(f"{label}: standalone playchar_title_box_put OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/m_char.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v638-playchar-title-box-put-group-{label}", "th04/m_char.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained playchar_title_box_put changed grouped OP_01_TEXT")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable_keys):
        raise RuntimeError("independent OP playchar_title_box_put cold rounds differ")
    current_hashes = {
        "src/op/menu/playchar_title_box_put.cpp": sha_file(SOURCE),
        "src/op/menu/playchar_title_box_put.inl": sha_file(BODY),
        **{path: sha_file(ROOT / path) for path in dependencies},
    ...
    if current_hashes != source_hashes:
        raise RuntimeError("maintained playchar_title_box_put source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed playchar_title_box_put maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "limit": "Decoded 106-byte function only; v489 remains surrounding build scaffold. No packed-file or whole-OP exactness.",
    }
