#!/usr/bin/env python3
"""Cold-replace ZUN's FILE_WRITE library member after local FILE_ROPEN support."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local FILE_WRITE changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local FILE_WRITE changed MAP")
...
def main() -> int:
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned FILE_WRITE support replaces external library member after local FILE_ROPEN support; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "FILE_WRITE is library-origin support, not authored ZUN progress. Its full 0xA6 module is the target function owner, including the internal alignment NOP before the direct-write path. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
