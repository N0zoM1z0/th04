#!/usr/bin/env python3
"""Cold-replay maintained natural C++ for MAINE graph_fraction_of_million_put."""
...
SOURCE = ROOT / "src/maine/end/graph_fraction_of_million_put.inl"
START = 0xB97B
SIZE = 0x77
NEXT = START + SIZE
...
TARGET_FUNCTION_SHA = "b95f07643434a2576f43d7cc560c6c2f93f83f97800180c1dbf58652f4a496c9"
...
def validate_target(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("MAINE graph_fraction_of_million_put target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("MAINE graph_fraction_of_million_put Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A05
        or int(row["entry_offset"], 0) != 0x192B
        ...
        raise RuntimeError(f"MAINE graph_fraction_of_million_put Ghidra extent drift: {row!r}")
    required = (
    ...
    if any(item not in map_text for item in required):
        raise RuntimeError("MAINE graph_fraction_of_million_put MAP evidence drift")
    if not body.startswith(bytes.fromhex("c8 02 00 00 56 57")):
        raise RuntimeError("MAINE million fraction prologue drift")
    if body[-6:] != bytes.fromhex("5f 5e c9 c2 08 00"):
        raise RuntimeError("MAINE million fraction epilogue drift")
    return {
...
def overlay_source(work: Path) -> str:
    ...
    start_anchor = b"void pascal near graph_fraction_of_million_put(\n"
    end_anchor = b"\n\nvoid near sub_B9F2(void)\n"
    if data.count(start_anchor) != 1 or data.count(end_anchor) != 1:
        raise RuntimeError("graph_fraction_of_million_put replacement anchors drift")
    start = data.index(start_anchor)
    end = data.index(end_anchor, start)
    upstream.write_bytes(
        data[:start] + b'#include "src/maine/end/graph_fraction_of_million_put.inl"' + data[end:]
    )
...
def main() -> int:
    ...
    closure = source_closure(ROOT, ("src/maine/end/graph_fraction_of_million_put.inl",))
    ...
    if baseline.program_image[START:NEXT] != body:
        raise RuntimeError("v489 graph_fraction_of_million_put bytes differ from target")
    ...
    for label in ("a", "b"):
        ...
        obj.unlink()
        tcc(work, output, f"v708-million-fraction-{label}", "th04/gv.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained graph_fraction_of_million_put changed gv MAINE_01_TEXT")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: cold MAINE graph_fraction_of_million_put link/layout drift")
        builds[label] = {
    ...
    if stable(builds["a"]) != stable(builds["b"]):
        raise RuntimeError("independent graph_fraction_of_million_put cold rounds differ")
    if any(prior.sha_file(ROOT / name) != digest for name, digest in source_hashes.items()):
        raise RuntimeError("maintained graph_fraction_of_million_put source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "MAINE graph_fraction_of_million_put maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        ...
        "builds": builds,
        "limit": (
            "Decoded 119-byte graph_fraction_of_million_put only. The surrounding gv.cpp is pinned "
