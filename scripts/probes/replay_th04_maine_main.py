#!/usr/bin/env python3
"""Cold-replay maintained natural C++ for MAINE's 400-byte _main."""

from __future__ import annotations

import argparse
import csv
from datetime import datetime, timezone
import json
from pathlib import Path
import shutil
import subprocess
import sys

ROOT = Path(__file__).resolve().parents[2]
sys.path[0:0] = [str(ROOT / "scripts"), str(ROOT / "scripts/probes")]

from lib.omf import describe_omf
from lib.pc98 import parse_mz
from compact_op_maine_snapshot import copy_compact_snapshot
from probe_th04_maine_staff_full_cpp_v478 import segment_bytes
from probe_th04_maine_segment_topology_v470 import tcc
from probe_th04_maine_score_producers_v468 import RUNNER, RUNNER_SHA, run_checked
from replay_th04_zun_source_only import source_closure
import replay_th04_maine_score_insert as prior

SOURCE = ROOT / "src/maine/end/main.inl"
START = 0xA102
SIZE = 0x190
NEXT = START + SIZE
PRODUCER_START = 0xA059
PRODUCER_SIZE = 0x239
LOCAL_START = START - PRODUCER_START
TARGET_FUNCTION_SHA = "9a44bbf57b6d60fa16f8a78415d969b62c6bf1c596c36fc20a779749cedd92a8"
TARGET_PRODUCER_SHA = "2dc26bd755036b3f18a9616b09e66f1bf0854ffffe2dcce4757573e77df286d3"
BASE_ENTRY_SOURCE_SHA = "12b2691adb49441da38573de5912938176e229fbd69a148eb747275e2a5128f6"
BASE_WRAPPER_SHA = "01e1e5ad494a8ed534080c4404f0f73e7fdaf1621e60ecbe8286c3ae1d0736af"
BASE_OBJECT_SHA = "561dbd875436121f4357763a6bd49b22ef087f0247ac460fff70e35d646f3c4e"
BASE_CODE_SHA = "e674aa8c05279f7a1f1f5d1c3eaee82928d193f36b14c901b5ef039af80947f1"


def sha(data: bytes) -> str:
    return prior.sha(data)


def validate_target(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("MAINE _main target identity drift")
    inv = ROOT / ".analysis/ghidra/boundary-exports/th04-maine/functions.csv"
    with inv.open(newline="", encoding="utf-8") as stream:
        rows = [r for r in csv.DictReader(stream)
                if int(r["entry_linear"], 0) == 0x10000 + START]
    if len(rows) != 1:
        raise RuntimeError("MAINE _main Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A05
        or int(row["entry_offset"], 0) != 0x00B2
        or int(row["body_min_linear"], 0) != 0x10000 + START
        or int(row["body_max_linear"], 0) != 0x10000 + NEXT - 1
        or int(row["body_addresses"]) != SIZE
        or int(row["body_span"]) != SIZE
        or row["contiguous"] != "true"
        or row["body_range_count"] != "1"
        or int(row["caller_count"]) != 1
        or int(row["callee_count"]) != 22
    ):
        raise RuntimeError(f"MAINE _main Ghidra extent drift: {row!r}")
    if "0A05:00B2       _main" not in map_text:
        raise RuntimeError("MAINE _main MAP entry drift")
    if "0A05:0009 0239 C=CODE   S=MAINE_E_TEXT" not in map_text:
        raise RuntimeError("MAINE_E_TEXT MAP contribution drift")
    if "0A05:0242 0000 C=CODE   S=CUTSCENE_TEXT" not in map_text:
        raise RuntimeError("MAINE next segment boundary drift")
    if not body.startswith(b"\xC8\x04\x00\x00") or body[-2:] != b"\xC9\xCB":
        raise RuntimeError("MAINE _main prologue/epilogue drift")
    return {
        "segment_identity": "1A05",
        "segment_offset": "00B2",
        "payload_offset": hex(START),
        "size": SIZE,
        "target_sha256": sha(body),
        "caller_count": 1,
        "callee_count": 22,
        "terminal": "leave; retf",
        "producer": "1A05:0009 MAINE_E_TEXT size 0x239",
        "next_segment": "1A05:0242 CUTSCENE_TEXT",
    }


def overlay_source(work: Path) -> str:
    entry = work / "th04/end/entry.cpp"
    wrapper = work / "th04/maine_e.cpp"
    if prior.sha_file(entry) != BASE_ENTRY_SOURCE_SHA:
        raise RuntimeError("pinned v489 ending entry translation source drift")
    if prior.sha_file(wrapper) != BASE_WRAPPER_SHA:
        raise RuntimeError("pinned v489 maine_e wrapper drift")
    shutil.copytree(ROOT / "src/maine/end", work / "src/maine/end", dirs_exist_ok=True)
    data = entry.read_bytes()
    anchor = b"void main(void)\n{"
    if data.count(anchor) != 1:
        raise RuntimeError("MAINE main replacement anchor is not unique")
    start = data.index(anchor)
    entry.write_bytes(data[:start] + b'#include "src/maine/end/main.inl"\n')
    return prior.sha_file(entry)


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--output-dir", type=Path, required=True)
    args = ap.parse_args()
    output = args.output_dir.resolve()
    private = (ROOT / ".analysis/reconstruction/probes").resolve()
    if output.exists() or output == private or not output.is_relative_to(private):
        ap.error("output must be new below .analysis/reconstruction/probes")

    subprocess.run([sys.executable, "scripts/preflight.py"], cwd=ROOT,
                   capture_output=True, text=True, check=True)
    if prior.sha_file(RUNNER) != RUNNER_SHA:
        raise RuntimeError("pinned DOS runner drift")
    for path, expected in (
        (prior.TARGET, prior.TARGET_SHA),
        (prior.SNAPSHOT / "bin/th04/maine.exe", prior.BASE_EXE_SHA),
        (prior.SNAPSHOT / "obj/th04/maine.map", prior.BASE_MAP_SHA),
        (prior.SNAPSHOT / "th04/end/entry.cpp", BASE_ENTRY_SOURCE_SHA),
        (prior.SNAPSHOT / "th04/maine_e.cpp", BASE_WRAPPER_SHA),
        (prior.SNAPSHOT / "obj/th04/maine_e.obj", BASE_OBJECT_SHA),
    ):
        if prior.sha_file(path) != expected:
            raise RuntimeError(f"pinned input identity drift: {path}")

    closure = source_closure(ROOT, ("src/maine/end/main.inl",))
    source_hashes = {name: prior.sha_file(ROOT / name) for name in closure}
    target = parse_mz(prior.TARGET.read_bytes())
    baseline = parse_mz((prior.SNAPSHOT / "bin/th04/maine.exe").read_bytes())
    if not target.valid or not baseline.valid or len(target.relocations) != prior.RELOCATIONS:
        raise RuntimeError("invalid target restore or v489 MAINE candidate")

    body = target.program_image[START:NEXT]
    target_producer = target.program_image[PRODUCER_START:PRODUCER_START + PRODUCER_SIZE]
    map_text = (prior.SNAPSHOT / "obj/th04/maine.map").read_text(encoding="cp437")
    boundary = validate_target(body, map_text)
    if sha(target_producer) != TARGET_PRODUCER_SHA:
        raise RuntimeError("MAINE_E_TEXT target producer identity drift")
    if baseline.program_image[START:NEXT] != body:
        raise RuntimeError("v489 MAINE _main bytes differ from target")
    if baseline.program_image[PRODUCER_START:PRODUCER_START + PRODUCER_SIZE] != target_producer:
        raise RuntimeError("v489 MAINE_E_TEXT producer differs from target")
    target_sites = [item.linear for item in target.relocations]
    if [item.linear for item in baseline.relocations] != target_sites:
        raise RuntimeError("v489 ordered relocations differ from target")

    base_object = prior.SNAPSHOT / "obj/th04/maine_e.obj"
    base_omf = describe_omf(base_object.read_bytes())
    base_code = segment_bytes(base_object, "MAINE_E_TEXT")
    if (
        not base_omf["valid"]
        or "TC86 Borland C++ 4.02" not in base_omf["translator_comments"]
        or len(base_code) != PRODUCER_SIZE
        or sha(base_code) != BASE_CODE_SHA
    ):
        raise RuntimeError("pinned v489 MAINE_E_TEXT object drift")

    output.mkdir(parents=True)
    builds: dict[str, dict[str, object]] = {}
    for label in ("a", "b"):
        work = output / label / "maine/source"
        work.parent.mkdir(parents=True)
        compact = copy_compact_snapshot(prior.SNAPSHOT, work, "maine")
        patched_source_sha = overlay_source(work)

        obj = work / "obj/th04/maine_e.obj"
        obj.unlink()
        tcc(work, output, f"v697-maine-main-group-{label}", "th04/maine_e.cpp")
        omf = describe_omf(obj.read_bytes())
        code = segment_bytes(obj, "MAINE_E_TEXT")
        if (
            not omf["valid"]
            or "TC86 Borland C++ 4.02" not in omf["translator_comments"]
            or code != base_code
        ):
            raise RuntimeError(f"{label}: maintained _main changed grouped MAINE_E_TEXT")

        exe = work / "bin/th04/maine.exe"
        mp = work / "obj/th04/maine.map"
        exe.unlink()
        mp.unlink()
        run_checked(["wine", str(RUNNER), "-e", "-x", "tlink", r"@obj\th04\maine.@l"],
                    work, output / f"link-{label}.log")
        image = parse_mz(exe.read_bytes())
        linked = image.program_image[START:NEXT]
        linked_producer = image.program_image[PRODUCER_START:PRODUCER_START + PRODUCER_SIZE]
        if (
            not image.valid
            or prior.sha_file(exe) != prior.BASE_EXE_SHA
            or prior.sha_file(mp) != prior.BASE_MAP_SHA
            or [item.linear for item in image.relocations] != target_sites
            or linked != body
            or linked_producer != target_producer
        ):
            raise RuntimeError(f"{label}: cold MAINE _main link/layout drift")
        builds[label] = {
            "compact_snapshot": compact,
            "patched_entry_source_sha256": patched_source_sha,
            "group_omf_sha256": prior.sha_file(obj),
            "group_code_sha256": sha(code),
            "linked_exe_sha256": prior.sha_file(exe),
            "linked_map_sha256": prior.sha_file(mp),
            "linked_program_sha256": sha(image.program_image),
            "linked_function_sha256": sha(linked),
            "linked_producer_sha256": sha(linked_producer),
            "raw_function_difference_count": 0,
            "raw_producer_difference_count": 0,
            "ordered_relocations": len(target_sites),
        }

    stable = lambda d: {k: v for k, v in d.items() if k != "group_omf_sha256"}
    if stable(builds["a"]) != stable(builds["b"]):
        raise RuntimeError("independent MAINE _main cold rounds differ")
    if any(prior.sha_file(ROOT / name) != digest for name, digest in source_hashes.items()):
        raise RuntimeError("maintained MAINE _main source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "MAINE target-reviewed _main and maintained natural-C++ cold replay",
        "artifact": "th04-maine",
        "target_restored_sha256": prior.TARGET_SHA,
        "baseline_exe_sha256": prior.BASE_EXE_SHA,
        "baseline_map_sha256": prior.BASE_MAP_SHA,
        "baseline_maine_e_object_sha256": BASE_OBJECT_SHA,
        "baseline_maine_e_code_sha256": BASE_CODE_SHA,
        "source_sha256": source_hashes,
        "driver_sha256": prior.sha_file(Path(__file__).resolve()),
        "boundary": boundary,
        "producer": {
            "segment": "MAINE_E_TEXT",
            "payload_offset": hex(PRODUCER_START),
            "size": PRODUCER_SIZE,
            "local_function_offset": hex(LOCAL_START),
            "target_linked_sha256": sha(target_producer),
            "raw_target_difference_count": 0,
        },
        "builds": builds,
        "limit": (
            "Decoded 400-byte _main only; no packed-file or whole-MAINE exactness. "
            "The surrounding pinned v489 tree remains replay scaffold."
        ),
    }
    path = output / "receipt.json"
    path.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({
        "receipt": str(path),
        "receipt_sha256": prior.sha_file(path),
        "function_sha256": TARGET_FUNCTION_SHA,
        "source_sha256": source_hashes,
    }, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
