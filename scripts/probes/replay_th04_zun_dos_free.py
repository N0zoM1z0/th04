#!/usr/bin/env python3
"""Cold-replace ZUN's DOS_FREE library member after local FILE_READ support."""
...
def link(label: str, output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"zun-filread-{label}-", dir=output) as temporary:
        ...
        if component != original_component or sha(component) != BASELINE_COMPONENT_SHA256:
            raise RuntimeError(f"{label}: local DOS_FREE changed component bytes")
        if map_data != original_map:
            raise RuntimeError(f"{label}: local DOS_FREE changed MAP")
...
def main() -> int:
    ...
    for source in (LOCAL_GRAPH_CLEAR, LOCAL_RESDATA, LOCAL_FILE_READ, LOCAL_DOS_FREE):
    ...
    receipt = {
        "schema_version": 1,
        "claim_scope": "repository-owned DOS_FREE support replaces external library member after local FILE_READ support; no authored or target exact claim",
        ...
        "exact": differences == 0,
        "limit": "DOS_FREE is library-origin support, not authored ZUN progress. External support, composite inputs, and the blocked resident layout remain; full packed artifact exact acceptance is open.",
    }
    if receipt["exact"]:
