...
| --- | ---: | ---: | ---: | ---: | --- |
| OP.EXE | 93 | 24 / 61 / 8 | 17 | 76 | Four hi_view functions accepted; registration menu is next reviewed owner |
...
The non-MAIN authored backlog is 141 pending acceptances, 15 provisional
...
OP additionally has 80-byte stage, 293-byte two-column row, 122-byte
rank renderers, and the 180-byte clear-state/sprite initializer. Neither artifact, nor ZUN, has
...
All OP/MAINE backends now copy only the source and artifact link-input closure
from the retained v489 snapshot: about 11 MiB per A/B worktree instead of
about 27 MiB. The wrapper also prunes successful generated worktrees after
each decoded raw comparison while retaining receipts and logs. Use
`--keep-workdirs` only for a bounded debugging run; failed backend worktrees
remain for diagnosis. Full v554 OP/MAINE aggregate replays preserved the
...
are deleted.
The final v554 wrapper replays checked OP 17/17 and MAINE 20/20 accepted
decoded slices raw-zero in two rounds; their receipt directories are 1.4 MiB
and 1.6 MiB. The ZUN control still reports 13/130 raw differences for its two
...
Seven MAINE SCORE functions bring MAINE to 20 / 1,962; four OP SCORE functions
bring OP to 17 / 1,515. Continue adjacent SCORE
...
- v554 accepts OP's 180-byte private clear-state/sprite initializer at `1A74:24A3`, payload `0xCBE3..0xCC96`, ending `RET`. The function has no own candidate MAP public; target/Ghidra disassembly and grouped SCORE_TEXT ownership establish its extent. Maintained C++ matches standalone CODE outside one near-call and 25 data-address OMF fixup words, then links raw-zero in two cold rounds with 804 ordered relocations. The OP wrapper now covers 17 slices. Packed OP, whole SCORE TU, and the adjacent registration menu remain unaccepted. See the [OP hi_view note](reconstruction/op-maine/TH04_OP_HI_VIEW_BOUNDARIES_V542.md#v554-maintained-private-clear-state-and-sprite-initializer).
