...
- The latest complete native aggregate is
  `gpt-web-item-overflow-aggregate-001`: all 253 selected MAIN owners pass two
  cold builds, raw bytes, MAP, ordered relocations, and OMF. Receipt SHA-256:
  `61f8414f9e5e0a05994cb7b3fac8d6b72fa3ce8c60e08439bd2ec65cda6e9f93`.
...
- The TH04 product tree now uses local runtime, graphics, platform, GRCG,
  ...
  geometry, and item-overflow interfaces. The remaining compatibility boundary
  is **10 forwarders, 11 include sites, and 11 product files**, with no missing, unused, invalid, or
...
1. **Finish standalone source closure.** Follow the
   ...
   beginning with `th02/main/midboss/midboss.hpp` and
   `th02/main/scroll.hpp`. Run a complete default aggregate after each
...
- `gpt-web-item-overflow-aggregate-001`
