#!/usr/bin/env python3
"""Cold-compile maintained OP bgm_choice_put and relink OP_SETUP_TEXT."""
...
SOURCE = ROOT / "src/op/setup/bgm_choice_put.cpp"
BODY = ROOT / "src/op/setup/bgm_choice_put.inl"
START = 0xB698
SIZE = 0x4F
NEXT = START + SIZE
LOCAL_START = START - PRODUCER_START
TARGET_FUNCTION_SHA = "3ecc90a3839d8f10c8ea88eebea2ec172c46e163442251c44d089231d056ab49"


def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP bgm_choice_put target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("OP bgm_choice_put Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x0F58
        ...
        raise RuntimeError(f"OP bgm_choice_put Ghidra extent drift: {row!r}")

    if (
        body[:8] != bytes.fromhex("c8 04 00 00 56 be 88 00")
        or body[8:25] != bytes.fromhex(
            "8b 46 06 0b c0 74 21 3d 01 00 74 0f 3d 02 00 75 22"
        )
        or body[25:35] != bytes.fromhex("8c 5e fe c7 46 fc b3 0d eb 18")
        or body[35:48] != bytes.fromhex("8c 5e fe c7 46 fc c4 0d 83 c6 10 eb 0b")
        or body[48:59] != bytes.fromhex("8c 5e fe c7 46 fc d5 0d 83 c6 20")
        or body[59:69] != bytes.fromhex("6a 30 56 ff 76 04 66 ff 76 fc")
        or body[69] != 0x9A
        or int.from_bytes(body[70:72], "little") != 0x04A4
        or int.from_bytes(body[72:74], "little") != 0x0DA1
        or body[74:] != bytes.fromhex("5e c9 c2 04 00")
    ):
        raise RuntimeError("OP bgm_choice_put instruction/operand topology drift")

    if (
        "0A74:0F58 idle  bgm_choice_put(int,unsigned int)" not in map_text
        or "0DA1:04A4       GRAPH_PUTSA_FX" not in map_text
    ):
        raise RuntimeError("OP bgm_choice_put MAP target drift")
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void pascal near bgm_choice_put(int bgm_mode, vc2 col)\n"
    end_marker = b"\n\nvoid pascal near se_choice_put(int se_mode, vc2 col)\n"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("OP bgm_choice_put source anchors drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start)
    replacement = b'#include "src/op/setup/bgm_choice_put.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/setup/bgm_choice_put.cpp": sha_file(SOURCE),
        "src/op/setup/bgm_choice_put.inl": sha_file(BODY),
    }
    ...
    expected_strings = {
        0x367: bytes.fromhex("83 58 83 65 83 8c 83 49 82 65 82 6c 89 b9 8c b9 00"),
        0x378: bytes.fromhex("81 40 95 57 8f 80 82 65 82 6c 89 b9 8c b9 81 40 00"),
        0x389: bytes.fromhex("81 40 81 40 89 b9 8a 79 96 b3 82 b5 81 40 81 40 00"),
    }
    ...
    for offset, expected in expected_strings.items():
        if base_data[offset:offset + len(expected)] != expected:
            raise RuntimeError(f"pinned BGM choice string drift at {offset:#x}")
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v624-bgm-choice-standalone-{label}", "src/op/setup/bgm_choice_put.cpp")
        standalone = [
            path for path in (work / "obj/th04").glob("*.obj")
            if path.name not in before
        ]
        if len(standalone) != 1:
            raise RuntimeError(f"{label}: expected one standalone bgm-choice object")
        ...
        target_unlinked = bytes.fromhex(
            "c8 04 00 00 56 be 88 00 8b 46 06 0b c0 74 21 3d 01 00 "
            "74 0f 3d 02 00 75 22 8c 5e fe c7 46 fc 00 00 eb 18 "
            "8c 5e fe c7 46 fc 1c 00 83 c6 10 eb 0b 8c 5e fe "
            "c7 46 fc 38 00 83 c6 20 6a 30 56 ff 76 04 66 ff "
        ...
        if (
            ...
            or local_fixups != [(3, 70), (1, 54), (1, 41), (1, 31)]
        ):
            raise RuntimeError(f"{label}: standalone OP bgm_choice_put OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_setup.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v624-bgm-choice-group-{label}", "th04/op_setup.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained bgm_choice_put changed grouped OP_SETUP_TEXT/OMF")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: linked OP bgm_choice_put/layout drift")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable):
        raise RuntimeError("independent OP bgm_choice_put cold rounds differ")
    if (
        sha_file(SOURCE) != source_hashes["src/op/setup/bgm_choice_put.cpp"]
        or sha_file(BODY) != source_hashes["src/op/setup/bgm_choice_put.inl"]
    ):
        raise RuntimeError("maintained OP bgm_choice_put source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed bgm_choice_put maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "builds": builds,
        "limit": "Decoded 79-byte function only; no DIET-packed or whole-OP exactness.",
    }
