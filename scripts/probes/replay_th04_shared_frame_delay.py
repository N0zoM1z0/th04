#!/usr/bin/env python3
"""Cold-relink OP and MAINE with the maintained shared frame-delay TU.
...
SOURCE = ROOT / "src/shared/hardware/frame_delay.cpp"
...
ARTIFACTS = {
    "op": (0xDA3B, 0x15, 804, "v228-op-target-roundtrip/a/restored.bin",
    ...
    "maine": (0xCCA3, 0x15, 559, "v228-maine-target-roundtrip/a/restored.bin",
...
def contribution(path: Path) -> tuple[int, int, str]:
    pattern = re.compile(
        ...
        r"\bS=SHARED\b.*\bM=th02/frmdely1\.cpp(?:\s|$)", re.I,
    )
    matches = []
    ...
    if len(matches) != 1:
        raise RuntimeError(f"expected one frame-delay MAP contribution: {path}")
    return matches[0]
...
def main() -> int:
    ...
    for label in ("a", "b"):
        ...
        for name, (start, size, reloc_count, _, _, baseline_sha) in ARTIFACTS.items():
            ...
            shutil.copy2(SOURCE, work / "th02/frmdely1.cpp")
            ...
            obj = work / "obj/th02/frmdely1.obj"
            obj.unlink()
            command = ["wine", str(RUNNER), "-e", "-x", "tcc", "-c", "-I.",
                       "-O", "-b-", "-3", "-Z", "-d", "-DGAME=4", "-ml",
                       "-nobj/th02/", "th02/frmdely1.cpp"]
            ...
            if differences or candidate.program_image != baselines[name].program_image:
                raise RuntimeError(f"{label}/{name}: frame-delay function or aggregate program differs")
    ...
    receipt = {
        ...
        "claim_scope": "artifact-local decoded frame_delay function raw replay",
