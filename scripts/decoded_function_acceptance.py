...
OP_NOPOLY_B_PUT_PRODUCER = 0xBED5
OP_NOPOLY_B_PUT_SIZE = 0x6A5
...
def validate(
    ...
    *,
    ...
    for entry in entries:
        ...
        if artifact == "th04-zun":
        ...
        elif backend == "maine-cutscene-animate-v687":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/cutscene/cutscene_animate.inl"
        ...
        elif backend == "maine-pic-put-both-masked-v691":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/cutscene/pic_put_both_masked.inl"
        ...
        elif backend == "maine-main-v697":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/end/main.inl"
                    or producer_start != MAINE_MAIN_PRODUCER
                    or producer_size != 0x239):
                raise ValueError(f"{ident}: MAINE main backend does not compile this producer")
        elif backend == "maine-staff-bg-helper-v700":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/end/staff_bgimage_expand_put.inl"
                    or producer_start != MAINE_STAFF_BG_HELPER_PRODUCER
                    or producer_size != 0x8B7):
                raise ValueError(f"{ident}: MAINE staff-bg-helper backend does not compile this producer")
        elif backend == "maine-staffroll-animate-v702":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/end/staffroll_animate.inl"
        ...
        elif backend == "maine-million-fraction-v708":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/end/graph_fraction_of_million_put.inl"
                    or producer_start != MAINE_GV_PRODUCER
                    or producer_size != 0x3FA):
                raise ValueError(f"{ident}: MAINE million-fraction backend does not compile this producer")
        elif backend == "maine-sub-b81d-v711":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/end/sub_B81D.inl"
                    or producer_start != MAINE_GV_PRODUCER
                    or producer_size != 0x3FA):
        ...
        elif backend == "maine-verdict-owner-v717":
            if (artifact != "th04-maine"
                    or source_name not in {
                        "src/maine/end/sub_BB81.inl",
                        "src/maine/end/verdict_animate.inl",
                    }
                    or producer_start != 0xBB81
                    or producer_size != 0x5C8):
                raise ValueError(f"{ident}: MAINE verdict-owner backend does not compile this producer")
        elif backend == "maine-score-rect-natural-v723":
            if (artifact != "th04-maine"
                    or source_name != "src/maine/score/score_rect_copy.inl"
                    or producer_start != 0xC149
                    or producer_size != 0xB30):
                raise ValueError(f"{ident}: MAINE natural SCORE rectangle backend does not compile this producer")
        ...
        elif backend == "op-mchar-remaining-v738":
            expected = {
                "src/op/menu/raise_bg_allocate_and_snap.inl",
                "src/op/menu/raise_bg_put.inl",
                "src/op/menu/pic_put.inl",
                "src/op/menu/shottype_titles_put.inl",
                "src/op/menu/shottype_title_box_put.inl",
            }
            if (artifact != "th04-op"
                    or source_name not in expected
                    or producer_start != 0xCF5E
                    or producer_size != 0xAB3):
                raise ValueError(f"{ident}: OP m-char remaining backend does not compile this producer")
        elif backend == "op-main-remaining-v742":
            expected = {
        ...
        elif backend == "op-zunsoft-natural-v753":
            expected = {
            ...
            if (artifact != "th04-op"
                    or source_name not in expected
                    or producer_start != 0xBA45
                    or producer_size != 0x490):
                raise ValueError(f"{ident}: OP ZUNSOFT natural backend does not compile this producer")
        ...
        elif backend == "op-start-game-v666":
            if (artifact != "th04-op"
                    or source_name != "src/op/start/start_game.cpp"
                    or producer_start != OP_START_GAME_PRODUCER
                    or producer_size != 0xD53):
                raise ValueError(f"{ident}: OP start-game backend does not compile this producer")
        elif backend == "op-cfg-save-exit-v668":
            if (artifact != "th04-op"
                    or source_name != "src/op/config/cfg_save_exit.cpp"
        ...
        elif backend == "op-cfg-load-v680":
            if (artifact != "th04-op"
                    or source_name != "src/op/config/cfg_load.cpp"
                    or producer_start != OP_CFG_LOAD_PRODUCER
                    or producer_size != 0xD53):
                raise ValueError(f"{ident}: OP cfg-load backend does not compile this producer")
        elif backend == "op-game-init-op-v671":
            if (artifact != "th04-op"
                    or source_name != "src/shared/core/game_init_op.cpp"
                    or producer_start != OP_GAME_INIT_OP_PRODUCER
                    or producer_size != 0x82):
                raise ValueError(f"{ident}: OP game-init-op backend does not compile this producer")
        elif backend == "op-menu-sel-update-v664":
            if (artifact != "th04-op"
                    or source_name != "src/op/main/menu_sel_update.cpp"
                    or producer_start != OP_MENU_SEL_UPDATE_PRODUCER
                    or producer_size != 0xD53):
                raise ValueError(f"{ident}: OP menu-selection-update backend does not compile this producer")
        ...
        elif backend == "op-egc-start-hybrid-v824":
            if (artifact != "th04-op"
                    or source_name != "src/op/hardware/egc_start_copy.inl"
                    or producer_start != OP_EGC_START_PRODUCER
                    or producer_size != OP_EGC_START_SIZE):
                raise ValueError(f"{ident}: OP EGC-start hybrid backend does not compile this producer")
        elif backend == "op-egc-copy-hybrid-v825":
            if (artifact != "th04-op"
                    or source_name != "src/op/hardware/egc_copy_rect_1_to_0_16.inl"
                    or producer_start != OP_EGC_START_PRODUCER
                    or producer_size != OP_EGC_START_SIZE):
                raise ValueError(f"{ident}: OP EGC-copy hybrid backend does not compile this producer")
        elif backend == "op-nopoly-b-put-hybrid-v826":
            if (artifact != "th04-op"
                    or source_name != "src/op/music/nopoly_put.inl"
...
def backend_command(backend_id: str, saved: Path, *, artifact: str | None = None) -> list[str]:
    ...
    if backend_id == "op-score-codecs-hybrid-v821":
        return [sys.executable, "scripts/probes/replay_th04_op_score_codecs_hybrid.py",
                "--output-dir", str(saved)]
    if backend_id == "op-egc-start-hybrid-v824":
        return [sys.executable, "scripts/probes/replay_th04_op_egc_start_copy_hybrid_v824.py",
                "--output-dir", str(saved)]
    if backend_id == "op-egc-copy-hybrid-v825":
        return [sys.executable, "scripts/probes/replay_th04_op_egc_copy_rect_hybrid_v825.py",
                "--output-dir", str(saved)]
    if backend_id == "op-nopoly-b-put-hybrid-v826":
        return [sys.executable, "scripts/probes/replay_th04_op_nopoly_b_put_hybrid_v826.py",
                "--output-dir", str(saved)]
    ...
    if backend_id == "op-big-menu-title-v735":
        return [sys.executable, "scripts/probes/replay_th04_op_big_menu_title.py",
                "--output-dir", str(saved)]
    if backend_id == "op-mchar-remaining-v738":
        return [sys.executable, "scripts/probes/replay_th04_op_mchar_remaining.py",
                "--output-dir", str(saved)]
    ...
    if backend_id == "op-music-remaining-v748":
        return [sys.executable, "scripts/probes/replay_th04_op_music_remaining.py",
                "--output-dir", str(saved)]
    if backend_id == "op-zunsoft-natural-v753":
        return [sys.executable, "scripts/probes/replay_th04_op_zunsoft_natural.py",
                "--output-dir", str(saved)]
    ...
    if backend_id == "maine-pi-put-quarter-v695":
        return [sys.executable, "scripts/probes/replay_th04_maine_pi_put_quarter.py",
                "--output-dir", str(saved)]
    if backend_id == "maine-main-v697":
        return [sys.executable, "scripts/probes/replay_th04_maine_main.py",
                "--output-dir", str(saved)]
    if backend_id == "maine-staff-bg-helper-v700":
        return [sys.executable, "scripts/probes/replay_th04_maine_staff_bgimage_expand_put.py",
                "--output-dir", str(saved)]
    ...
    if backend_id == "maine-skill-percentage-v706":
        return [sys.executable, "scripts/probes/replay_th04_maine_skill_percentage.py",
                "--output-dir", str(saved)]
    if backend_id == "maine-million-fraction-v708":
        return [sys.executable, "scripts/probes/replay_th04_maine_graph_fraction.py",
                "--output-dir", str(saved)]
    ...
    if backend_id == "maine-sub-b9f2-v714":
        return [sys.executable, "scripts/probes/replay_th04_maine_sub_b9f2.py",
                "--output-dir", str(saved)]
    if backend_id == "maine-verdict-owner-v717":
        return [sys.executable, "scripts/probes/replay_th04_maine_verdict_owner.py",
                "--output-dir", str(saved)]
    if backend_id == "maine-score-rect-natural-v723":
        return [sys.executable, "scripts/probes/replay_th04_maine_score_rect_natural.py",
                "--output-dir", str(saved)]
    ...
    if backend_id == "op-start-extra-v662":
        return [sys.executable, "scripts/probes/replay_th04_op_start_extra.py",
                "--output-dir", str(saved)]
    if backend_id == "op-start-game-v666":
        return [sys.executable, "scripts/probes/replay_th04_op_start_game.py",
                "--output-dir", str(saved)]
    ...
    if backend_id == "op-cfg-save-v673":
        return [sys.executable, "scripts/probes/replay_th04_op_cfg_save.py",
                "--output-dir", str(saved)]
    if backend_id == "op-cfg-load-v680":
        return [sys.executable, "scripts/probes/replay_th04_op_cfg_load.py",
                "--output-dir", str(saved)]
    if backend_id == "op-game-init-op-v671":
        return [sys.executable, "scripts/probes/replay_th04_op_game_init_op.py",
                "--output-dir", str(saved)]
    if backend_id == "op-menu-sel-update-v664":
        return [sys.executable, "scripts/probes/replay_th04_op_menu_sel_update.py",
                "--output-dir", str(saved)]
