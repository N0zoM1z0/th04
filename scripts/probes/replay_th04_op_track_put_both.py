#!/usr/bin/env python3
"""Cold-compile maintained OP track_put_both and relink OP_MUSIC_TEXT."""
...
RELOCATIONS = 804
SOURCE = ROOT / "src/op/music/track_put_both.cpp"
BODY = ROOT / "src/op/music/track_put_both.inl"
DEPENDENCIES = (
...
TARGET_FUNCTION_SHA = "e628575e98373909fbae9b962e8d57df747fa86fd482fc5eb59fc8beaedb755b"
...
def target_boundary(body: bytes, map_text: str) -> dict[str, object]:
    if len(body) != SIZE or sha(body) != TARGET_FUNCTION_SHA:
        raise RuntimeError("OP track_put_both target identity drift")
    ...
    if len(rows) != 1:
        raise RuntimeError("OP track_put_both Ghidra entry count drift")
    row = rows[0]
    if (
        int(row["entry_segment"], 0) != 0x1A74
        or int(row["entry_offset"], 0) != 0x1795
        ...
        raise RuntimeError(f"OP track_put_both Ghidra extent drift: {row!r}")

    if (
        body[:8] != bytes.fromhex("c8 02 00 00 b0 01 2a 06")
        or int.from_bytes(body[8:10], "little") != 0x3A7D
        or body[10:16] != bytes.fromhex("88 46 ff ba a6 00")
        or body[16:18] != bytes.fromhex("ee 6a")
        or body[18] != 0x10
        or int.from_bytes(body[50:52], "little") != 0x0EB6
        or body[52] != 0x9A
        or int.from_bytes(body[53:55], "little") != 0x04A4
        or int.from_bytes(body[55:57], "little") != 0x0DA1
        or body[57:61] != bytes.fromhex("ba a6 00 a0")
        or int.from_bytes(body[61:63], "little") != 0x3A7D
        or body[63] != 0xEE
        or int.from_bytes(body[97:99], "little") != 0x0EB6
        or body[99] != 0x9A
        or int.from_bytes(body[100:102], "little") != 0x04A4
        or int.from_bytes(body[102:104], "little") != 0x0DA1
        or body[-4:] != bytes.fromhex("c9 c2 04 00")
    ):
        raise RuntimeError("OP track_put_both instruction/operand topology drift")

    required = (
        "0A74:1795 idle  track_put_both(unsigned char,unsigned char)",
        "0DA1:04A4       GRAPH_PUTSA_FX",
        "0F34:0EB6 idle  _MUSIC_CHOICES",
        "0F34:3A7D idle  _music_page_accessed",
    )
    if any(item not in map_text for item in required):
        raise RuntimeError("OP track_put_both MAP target drift")
    ...
    return {
        ...
        "caller_count": 2,
        "callee_count": 1,
        "music_page_accessed": "0F34:3A7D",
        "music_choices": "0F34:0EB6",
        "graph_putsa_fx": "0DA1:04A4",
        "access_page_port": "0xA6",
        "coordinates": {"left": 16, "top": "8 + i*16"},
        "terminal": "RET 4",
    }
...
def overlay_source(work: Path) -> str:
    ...
    start_marker = b"void pascal near track_put_both(uint8_t i, vc_t col)\n"
    end_marker = b"\n\nvoid pascal near tracklist_put_both(uint8_t sel)\n"
    if data.count(start_marker) != 1 or data.count(end_marker) != 1:
        raise RuntimeError("track_put_both replacement anchor drift")
    start = data.index(start_marker)
    end = data.index(end_marker, start)
    replacement = b'#include "src/op/music/track_put_both.inl"'
...
def main() -> int:
    ...
    source_hashes = {
        "src/op/music/track_put_both.cpp": sha_file(SOURCE),
        "src/op/music/track_put_both.inl": sha_file(BODY),
    ...
    if "0A74:1795 idle  track_put_both(unsigned char,unsigned char)" not in map_text:
        raise RuntimeError("v489 track_put_both MAP public drift")
    ...
    for label in ("a", "b"):
        ...
        tcc_op(work, output, f"v636-track-put-both-standalone-{label}",
               "src/op/music/track_put_both.cpp")
        ...
        expected_fixups = [(3, 100), (1, 97), (1, 61), (3, 53), (1, 50), (1, 8)]
        ...
        if (
            ...
            raise RuntimeError(f"{label}: standalone track_put_both OMF/code drift")

        patched_source_sha = overlay_source(work)
        group_obj = work / "obj/th04/op_music.obj"
        group_obj.unlink()
        tcc_op(work, output, f"v636-track-put-both-group-{label}", "th04/op_music.cpp")
        ...
        if (
            ...
            raise RuntimeError(f"{label}: maintained track_put_both changed grouped OP_MUSIC_TEXT")
    ...
    if any(builds["a"][key] != builds["b"][key] for key in stable_keys):
        raise RuntimeError("independent OP track_put_both cold rounds differ")
    current_hashes = {
        "src/op/music/track_put_both.cpp": sha_file(SOURCE),
        "src/op/music/track_put_both.inl": sha_file(BODY),
        **{path: sha_file(ROOT / path) for path in DEPENDENCIES},
    ...
    if current_hashes != source_hashes:
        raise RuntimeError("maintained OP track_put_both source changed during replay")

    receipt = {
        "schema_version": 1,
        "observed_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "claim_scope": "OP target-reviewed track_put_both maintained-source cold-link replay",
        "artifact": "th04-op",
        ...
        "limit": "Decoded 108-byte function only; v489 remains surrounding build scaffold. No packed-file or whole-OP exactness.",
    }
