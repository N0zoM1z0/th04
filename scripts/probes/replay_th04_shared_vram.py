#!/usr/bin/env python3
"""Cold-relink OP and MAINE with the maintained shared VRAM-plane TU.
...
SOURCE = ROOT / "src/shared/hardware/vram_planes.cpp"
HEADER = ROOT / "src/shared/hardware/vram_planes.hpp"
...
ARTIFACTS = {
    "op": (0xDA12, 804, "v228-op-target-roundtrip/a/restored.bin",
    ...
    "maine": (0xCC7A, 559, "v228-maine-target-roundtrip/a/restored.bin",
...
def contribution(path: Path) -> tuple[int, int, str]:
    pattern = re.compile(
        ...
        r"\bS=SHARED\b.*\bM=th01/vplanset\.cpp(?:\s|$)", re.I,
    )
    matches = []
    ...
    if len(matches) != 1:
        raise RuntimeError(f"expected one vplanset MAP contribution: {path}")
    return matches[0]
...
def main() -> int:
    ...
    targets = {}
    baselines = {}
    for name, (start, reloc_count, target_rel, target_sha, baseline_sha) in ARTIFACTS.items():
        ...
        if len(targets[name].program_image[start:start + 0x29]) != 0x29:
            raise RuntimeError(f"{name}: truncated target function")
    ...
    for label in ("a", "b"):
        ...
        for name, (start, reloc_count, _, _, baseline_sha) in ARTIFACTS.items():
            ...
            if (map_start, map_size) != (start, 0x29):
                raise RuntimeError(f"{label}/{name}: source ownership moved")
            expected = target.program_image[start:start + 0x29]
            actual = candidate.program_image[start:start + 0x29]
            ...
            if differences or candidate.program_image != baselines[name].program_image:
                raise RuntimeError(f"{label}/{name}: VRAM function or aggregate program differs")
    ...
    receipt = {
        ...
        "claim_scope": "artifact-local decoded vram_planes_set function raw replay",
